...
REMOTE_LOG="/workspace/app/run_qhead_beta_rd3.log"
LOCAL_BASE="/home/jupyter/contrastive-forecasting/sync_qhead_beta_rd3"
...
mkdir -p "${LOCAL_CKPT}"
echo "[$(date)] Sync loop start qhead_beta_rd3 instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
...
while true; do
    ...
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === qhead_beta_rd3 cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    if grep -q "R3 ALL DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  === R3 ALL DONE detected; final sync ===" | tee -a "${LOG_FILE}"
        atomic_scp "${REMOTE_LOG}" "${LOCAL_BASE}/run.log" "${LOG_MIN}" || true
        echo "[$(date)] Sync loop qhead_beta_rd3 finished" | tee -a "${LOG_FILE}"
        exit 0
    fi
