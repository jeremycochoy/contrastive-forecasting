#!/bin/bash
# #359 — downstream q-head + GIFT-Eval for the SIGReg-B=512 enc3+CPC backbone
# with λ_embedding=1.0, λ_encoding=0.1. Identical protocol to #355's
# downstream_sigreg.sh (the comparison must be head-matched against #355,
# #353 EMA-target, and #344 enc3+CPC). Trains best-loss q-head (30k) then
# last-checkpoint q-head (resumed, 10k re-adapt), per head_layers, then runs
# full-97 GIFT-Eval for each cell.
#
#   downstream_sigreg.sh <head_layers: 2|6> <gpu>
set -uo pipefail
HL="${1:?head_layers}"; GPU="${2:?gpu}"
TAG="allt08_xftrip_nobn_enc3_emateach_sigreg_qk_aon_b512_cpc_emb10"
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [dl-sigreg-enc3 ${HL}L g$GPU] $*"; }
...
log "sigreg downstream complete (${HL}L)"
