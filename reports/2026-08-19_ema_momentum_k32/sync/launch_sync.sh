#!/bin/bash
# #404 — the sync loop for the box that trains the four backbones.
#
# CLAUDE.md, Remote Machine Monitoring: EVERY remote training run has a sync
# loop for its whole duration, short runs included, and the loop is verified by
# `ls` rather than by reading its own log.
#
...
#
# REMOTE_RUNS is the BOX's root, `CF404_BOX_RUNS`, and this study saves ONE
# LEVEL DEEPER than #373: `<root>/<arm>/<cell>/leg_<N>k/`, because
# `cf404_arm_root` adds the arm. `remote_listing` runs `find <dir> -type f`
# with no `-maxdepth` and `pull_tree` rebuilds the relative path, so any depth
# comes back.
#
# When this is NOT needed: an arm that trains on elisa writes straight to
# /home/jupyter/checkpoints_backup/cf-404, which is durable local disk on the
...
#   REMOTE_DIR=/root/contrastive-forecasting/reports/2026-08-19_ema_momentum_k32 \
#     bash sync/launch_sync.sh box_a
#
# The label names the local root, so two boxes never write one another's files.
# The script waits for the first tick and then lists what landed.
set -uo pipefail
...
LOOP="$CF404_PARENT/sync/sync_loop.sh"
SAFE_PULL="${SAFE_PULL:-$CF404_REPO/experiments/2026-04-27_periodic-synth-mix/scripts/safe_pull.sh}"
LOCAL_DIR="${LOCAL_DIR:-$CF404_SYNC_DIR}"
INTERVAL="${INTERVAL:-900}"
# REMOTE_RUNS is a directory on the BOX. `CF404_ROOT` is the LOCAL one — on
# elisa it is where this loop lands the tree — so a default taken from it would
# ask the box for elisa's own path and pull nothing.
REMOTE_RUNS="${REMOTE_RUNS:-$CF404_BOX_RUNS}"
case "$REMOTE_RUNS" in
  /root/*) ;;
  *) echo "ABORT: REMOTE_RUNS=$REMOTE_RUNS is not a path on the box." >&2
     echo "  The box saves under $CF404_BOX_RUNS." >&2
     exit 2 ;;
esac
...
# One loop per local root, identified by its working directory. Every loop runs
# the same command line and takes its target from the environment, so argv
# cannot tell two of them apart.
if [ "$(cf404_sync_loops "$LOCAL_DIR")" -ge 1 ]; then
  echo "[$LABEL] a loop already runs for $LOCAL_DIR, leaving it"
  exit 0
fi
