#!/bin/bash
# #404 — the steps that follow the round 4 driver, detached from any session.
#
# `round4.sh` trains the two added seeds, scores them on elisa and tears the
# box down. It never touches git, and it draws no figure. A session that ends
# takes its own waiters with it, so this script holds the rest, and it holds it
# under `nohup setsid`:
#
#   1. Wait for the driver to exit, by pid. Never by a pattern.
#   2. Put the eight arms' raw artefacts into the study directory.
...
#
# Step 5 repeats what the driver does. It is idempotent, and it is the safety
# net for a driver that dies before its own stage 9.
#
# WHAT THE COMMENT MUST CARRY. The card asks four things of this round, and
# `pr_comment.py --sync-root` answers all four from the tables on disk:
#
#   - the four scores of the s08 arm, at seeds 20260520 to 20260523
#   - how many of the four collapsed, BY THE AUC AT 40,000 STEPS
#   - the spread over the seeds that did NOT collapse, in absolute terms
#   - whether that spread separates 0.9 constant from 0.95 constant, whose gap
#     is 0.0088
#
# Usage:
#   nohup setsid bash scripts/finish_round4.sh \
#     > results/finish_round4.out 2>&1 < /dev/null &
#
#   ONLY=verify bash scripts/finish_round4.sh   # steps 2 to 4 alone, so the
...
ROUND_ARMS="${ROUND_ARMS:-s08c s08d}"
ENVF="${ENVF:-$CF404_RESULTS/round4.env}"
PIDF="${PIDF:-$CF404_RESULTS/round4.pid}"
WAIT_MAX="${WAIT_MAX:-50400}"   # 14 h, the driver's own deadline
ONLY="${ONLY:-all}"             # `verify` runs steps 2 to 4 and stops
LOG="$CF404_RESULTS/finish_round4.log"
VERIFY="$CF404_RESULTS/verify_round4.txt"
BODY="$CF404_RESULTS/pr_comment_round4.md"
BOX_LABEL_R4="${BOX_LABEL_R4:-box_r4}"

say(){ echo "[$(date '+%m-%d %H:%M:%S')] [#404 finish4] $*" | tee -a "$LOG"; }
...
if [ "$ONLY" = verify ]; then
  ...
  while kill -0 "$DRIVER" 2>/dev/null; do
    ...
    [ $(( waited % 1800 )) -eq 0 ] && [ "$waited" -gt 0 ] && \
      say "  driver up ${waited}s, $(tail -1 "$CF404_RESULTS/round4.log" 2>/dev/null | cut -c1-140)"
...
CF404_BOX_LABEL=box_a CF404_ROOT="$MAIN_ROOT" CF404_SYNC_ROOT="$MAIN_ROOT" \
  ...
  bash "$HERE/report_assets.sh" >>"$CF404_RESULTS/report_assets_finish4.out" 2>&1
say "  $(tail -1 "$CF404_RESULTS/report_assets_finish4.out" 2>/dev/null)"
...
CF404_ROOT="$MAIN_ROOT" CF404_SYNC_DIR="$MAIN_DIR" CF404_SYNC_TREE="$SYNC_TREE" \
  bash "$HERE/make_plots.sh" >>"$CF404_RESULTS/make_plots_finish4.out" 2>&1
say "  scores.csv holds $(( $(wc -l <"$CF404_RESULTS/scores.csv") - 1 )) arm(s)"
grep -E '^(wrote|SKIP)' "$CF404_RESULTS/make_plots_finish4.out" 2>/dev/null \
  | sed 's/^/  /' | tee -a "$LOG"

# ---- 4: the verification, by name and by size --------------------------------
say "verify"
{
  echo "#404 round 4 — every artefact of the eight arms, on elisa"
...
have="$(timeout 180 gh pr view "$PR" --json comments \
  --jq '[.comments[] | select(.body | contains("## The repeat family, seed by seed"))] | length' 2>/dev/null)"
if [ "${have:-0}" -ge 1 ] && [ "${FORCE_COMMENT:-0}" != 1 ]; then
  ...
  python3 "$HERE/pr_comment.py" --scores "$CF404_RESULTS/scores.csv" \
    ...
    --out "$BODY" >>"$CF404_RESULTS/pr_comment_round4.out" 2>&1 || {
      say "  pr_comment.py failed, see pr_comment_round4.out"; }
  if [ -s "$BODY" ]; then
    awk -v h="**Round 4 is complete: $n_round of 2 added seeds scored, $n_arms arms in all.**" \
...
exp(#404): four seeds of one arm, and what they do to every ranking

s08c (seed 20260522) and s08d (seed 20260523) repeat the s08 arm. With s08
(20260520) and s08b (20260521) the card now holds four backbone seeds of ONE
cell, so it can tell an unlucky seed from an unstable cell.

`seed_report.py` holds the study's one definition of a collapse: the
contrastive AUC at the stop, against a line at 0.80. The line sits in a wide
empty band, so it classifies no arm by accident. The figure and the tables read
that one definition, so they cannot disagree.

The repeat spread is measured over the seeds that did NOT collapse. The
distance between a collapsed run and a healthy one is not a spread, and quoting
it as one calls every arm of this card unranked.

`plots/backbone_health.png` carries the eight arms. `results/seed_report.md`
and `results/seed_table.csv` carry the four seeds, the collapse count, the
spread and the separation test. `verify_round4.txt` lists every artefact by
name and by size.
