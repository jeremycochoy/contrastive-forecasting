#!/bin/bash
# #404 — the steps that follow the round 6 driver, detached from any session.
#
# `round6.sh` trains `r100_09`, `r100_08` and `w3_s08`, starts each head as its
# own backbone lands, pulls every artefact, proves each one reads, destroys the
# box and then scores all three on elisa. It never touches git, and it draws no
# figure. A session that ends takes its own waiters with it, so this script
# holds the rest, under `nohup setsid`:
#
...
#
# WHAT THE COMMENT MUST CARRY. This round moves the L_align WEIGHT, and the
# round before it moved the ramp LENGTH. So the comment carries every scored
# arm with TWO values beside its name: the momentum it holds at the stop, and
# its align weight. Neither is on the arm's name. Four arms start at 0.8 or
# 0.9 and reach four different momenta, and two arms share every value except
# the weight.
#
...
#
# Usage:
#   nohup setsid bash scripts/finish_round6.sh \
#     > results/finish_round6.out 2>&1 < /dev/null &
#
#   ONLY=verify bash scripts/finish_round6.sh   # steps 2 to 4 alone
set -uo pipefail
...
ROUND_ARMS="${ROUND_ARMS:-r100_09 r100_08 w3_s08}"
ENVF="${ENVF:-$CF404_RESULTS/round6.env}"
PIDF="${PIDF:-$CF404_RESULTS/round6.pid}"
WAIT_MAX="${WAIT_MAX:-50400}"   # 14 h, the driver's own deadline
ONLY="${ONLY:-all}"             # `verify` runs steps 2 to 4 and stops
LOG="$CF404_RESULTS/finish_round6.log"
VERIFY="$CF404_RESULTS/verify_round6.txt"
BODY="$CF404_RESULTS/pr_comment_round6.md"
BOX_LABEL_R4="${BOX_LABEL_R4:-box_r4}"

say(){ echo "[$(date '+%m-%d %H:%M:%S')] [#404 finish6] $*" | tee -a "$LOG"; }
...
if [ "$ONLY" = verify ]; then
  ...
  while kill -0 "$DRIVER" 2>/dev/null; do
    ...
    [ $(( waited % 1800 )) -eq 0 ] && [ "$waited" -gt 0 ] && \
      say "  driver up ${waited}s, $(tail -1 "$CF404_RESULTS/round6.log" 2>/dev/null | cut -c1-140)"
...
CF404_BOX_LABEL=box_a CF404_ROOT="$MAIN_ROOT" CF404_SYNC_ROOT="$MAIN_ROOT" \
  ...
  bash "$HERE/report_assets.sh" >"$CF404_RESULTS/report_assets_finish6.out" 2>&1
say "  $(tail -1 "$CF404_RESULTS/report_assets_finish6.out" 2>/dev/null)"
...
CF404_ROOT="$MAIN_ROOT" CF404_SYNC_DIR="$MAIN_DIR" CF404_SYNC_TREE="$SYNC_TREE" \
  bash "$HERE/make_plots.sh" >"$CF404_RESULTS/make_plots_finish6.out" 2>&1
say "  scores.csv holds $(( $(wc -l <"$CF404_RESULTS/scores.csv") - 1 )) arm(s)"
grep -E '^(wrote|SKIP)' "$CF404_RESULTS/make_plots_finish6.out" 2>/dev/null \
  | sed 's/^/  /' | tee -a "$LOG"

# ---- 4: the verification, by name and by size --------------------------------
say "verify"
{
  echo "#404 round 6 — every artefact of every arm, on elisa"
...
# The key is this round's own heading, not round 5's. Every earlier round
# posted a comment carrying "## The repeat family, seed by seed", so that
# string would make this round think it had already posted.
have="$(timeout 180 gh pr view "$PR" --json comments \
  --jq '[.comments[] | select(.body | contains("Round 6 is complete"))] | length' 2>/dev/null)"
if [ "${have:-0}" -ge 1 ] && [ "${FORCE_COMMENT:-0}" != 1 ]; then
  ...
  python3 "$HERE/pr_comment.py" --scores "$CF404_RESULTS/scores.csv" \
    ...
    --cost "\$$SPENT on the round 4 box, one RTX 5090 with a Ryzen 7 7800X3D, three lanes on one card" \
    --out "$BODY" >"$CF404_RESULTS/pr_comment_round6.out" 2>&1 || {
      say "  pr_comment.py failed, see pr_comment_round6.out"; }
  if [ -s "$BODY" ]; then
    awk -v h="**Round 6 is complete: $n_round of 3 arms scored, $n_arms arms in all. \`w3_s08\` is \`s08\` with --align-loss-weight at 3.0.**" \
...
exp(#404): the align weight, and a third lane that shares one card

For this loss shape the rollout depth touches the align term alone. A depth
copy of the h-anchored repel term adds exactly zero, and the align add-on is
duplicated. The reduction is a mean, so 33 copies of L_align average back to
about one copy, and the loss holds one repel term against the mean of 33 pull
terms. --align-loss-weight is the only flag that sets that balance, and no arm
of rounds 1 to 5 moved it.

w3_s08 is s08, the best-scoring arm of this card at 1.1782, with that one flag
at 3.0 instead of 1.0. The momentum, the ramp, the depth and both seeds stay.

Column 6 of arms.tsv is the align weight. run_arm.sh appends the flag after
the cell's own, where argparse keeps it, and reads it back off the trainer's
command line beside alpha, the reduction and the seed. w3_s08 differs from s08
in that column alone, so a weight that missed the trainer would train a
duplicate under a second name.

Every reader keys on the weight. plot_momentum.py and momentum_at_stop.py put
it in the series key, repeat_spread.py and seed_report.py keep it out of the
repeat family, and scores.csv, table.md and the PR comment carry it as a
column.

The three lanes shared one RTX 5090 at 87 % and 17,042 MiB of 32,607 MiB.

verify_round6.txt lists every artefact by name and by size.
