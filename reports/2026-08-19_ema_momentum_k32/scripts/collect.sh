...
#                        arm's momentum, backbone seed and L_align weight
#                        beside it. Feeds the momentum figure and the table.
#                        The seed is a column because one arm of this card is a
#                        REPEAT of another at a second seed, and a reader of
#                        the table has to see which pair that is. The align
#                        weight is a column for the same reason: `w3_s08` is
#                        `s08` with that one flag moved, and the two rows are
#                        otherwise identical.
...
{
  echo "arm,alpha,schedule,ramp,seed,align_w,stop,head_steps,encoder,score"
  for f in "$CF404_RESULTS"/score_*.txt; do
    ...
    echo "$arm,$(cf404_alpha "$arm"),$(cf404_schedule "$arm"),$(cf404_ramp "$arm"),$(cf404_seed "$arm"),$(cf404_align_weight "$arm"),$stop,$head,$enc,$score"
