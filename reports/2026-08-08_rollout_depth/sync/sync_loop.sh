#!/bin/bash
# #373 persistent sync loop — 15-min ticks pulling this study's artefacts
...
REMOTE_RUNS="${REMOTE_RUNS:-/home/jupyter/checkpoints_backup/cf-373}"
...
SAFE_PULL="${SAFE_PULL:-$(dirname "$LOCAL_DIR")/2026-04-27_periodic-synth-mix/scripts/safe_pull.sh}"
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [sync-373] $*"; }
...
while true; do
  log "tick"
  # The whole runs root in one walk. A per-cell list would have to be kept
  # in step with the box assignment, and a stale entry drops a cell without
  # a failure line — the exact thing this file's header warns about.
  pull_tree "$REMOTE_RUNS" "$LOCAL_DIR/sync"
  # The study's own record: the per-run trainer logs and the launcher logs.
