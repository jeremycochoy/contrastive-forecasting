#!/bin/bash
# #373 round 3, session eleven — block until the round moves enough to report.
#
...
#
# It writes an hourly heartbeat to results/q_s11_heartbeat.log the whole time,
# so a dead session still leaves a record of where the round got to.
#
# Usage: bash q_await_s11.sh [max seconds] [poll seconds] [delta jobs]
set -uo pipefail
...
D0="$(ndone)"
log "s11 await starts. $(counts) wake at done=$(( D0 + DELTA )) or terminal"
