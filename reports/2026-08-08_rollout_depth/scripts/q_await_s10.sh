#!/bin/bash
# #373 round 3, session ten — block until the queue drains or breaks.
#
# q_await.sh returns on the FIRST queue move. The tail of this round moves
# every ~30 min, so that costs a turn per move and reads nothing new. This
# one blocks until the round is over, and only breaks out early for the four
# things a session must act on:
#
...
#   the deadline                 nothing to act on, say so    exit 7
#
# It writes an hourly heartbeat to results/q_s10_heartbeat.log the whole time,
# so a dead session still leaves a record of where the round got to.
#
# Usage: bash q_await_s10.sh [max seconds] [poll seconds]
set -uo pipefail
