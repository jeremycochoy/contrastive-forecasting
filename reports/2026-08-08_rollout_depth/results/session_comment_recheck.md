...
## Both flagged jobs were already finished. A second implementation now confirms them.

**Experiment directory:** `reports/2026-08-08_rollout_depth/` (`results/`, `plots/`, `scripts/`, `sync/`)

The review reported `G_B1_k0_aw4` with no eval and item 6 with no reseed log.
That was true when it was written. Both had landed by 22:40 the same evening and
are committed. This session ran no training and no eval. It re-derived both
results with code written from scratch, then checked the state the rules fix.
...
bb40k and bb100k, read off the coverage table's columns. B1's real k=0 baseline
is **1.2025 / 1.2001** (`G6_B1_k0_bb40k`). The -0.1175 runs 1.2025 → 1.0850, and
the control sits between the two.
...
95% paired dataset-cluster intervals over the 97 configs, from this session's
independent implementation:

- student: re-weighting **-0.0512 [-0.1011, -0.0017]** (97.8%), depth **-0.0663 [-0.1070, -0.0341]** (100%), total **-0.1175 [-0.1808, -0.0606]** (100%)
- teacher: re-weighting **-0.0519 [-0.0993, -0.0055]** (98.6%), depth **-0.0534 [-0.0880, -0.0240]** (100%), total **-0.1053 [-0.1667, -0.0508]** (100%)
...
|---|---|---|
| the re-weighting | -0.0009 [-0.0501, +0.0459] | -0.1400 [-0.2276, -0.0623] |
| the depth | -0.0547 [-0.0839, -0.0314] | -0.0845 [-0.1810, -0.0148] |

The control is clean, read off the trained artefact rather than the launcher.
Re-derived here from the three loss CSVs directly: the ×4 arm sits **+3.73116**
above k=0 at step 1 on a shared seed and batch order, which is 3 × `L_align(1)`,
so `L_align(1)` ≈ 1.2437. It writes **no `cos_err_d*` column**, so it trains no
rollout, while k=3 writes `cos_err_d0..d3`. All three arms logged 40,000 steps.
...
GPU 1 at seed 20260723, the eval merged 97 configs at 18:20:29, score committed
18:24. Both draws trained 30,000 head steps on the same backbone file.
...
|---|---|---|
| draw 2 against the original **1.3998** | **+0.0100** | [-0.0160, +0.0378] |
| draw 1 against the teacher **1.2913** | **+0.1084** | [+0.0661, +0.1632] |
| draw 2 against the teacher **1.2913** | **+0.1185** | [+0.0720, +0.1812] |
...
A check suite can agree with itself. `scripts/independent_recheck.py` re-derives
both results with separate code: numpy in place of the `random` module,
vectorised cluster resampling, bootstrap seed 7919373 in place of 20260809. It
reads `all_results.csv` and the seasonal-naive reference only, and touches no
score file until the comparison.

| what it re-derived | result |
|---|---|
| all 9 scores, from the per-config CSVs | **9 of 9 reproduce**, worst deviation 3.97e-05 against a 5.0e-05 allowance |
| all 9 observed deltas | **match to the last printed digit**, worst difference 0.0000 |
| all 27 interval bounds | **agree to Monte-Carlo noise**, no bound moves more than 0.0039 |
| which side of zero each interval sits on | **no disagreement in 15 of 15** |
| the ×4 flag, off the loss curves | **+3.73116** at step 1, no `cos_err_d*`, 40,000 steps |
| the denominator, across the 7 evals involved | **one identical `SN_MASE` column**, 97 rows |
