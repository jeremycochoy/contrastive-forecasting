...
#                  batch size and the L_rep decay, appended LAST to the trainer
#                  command line. The runner states the width and the batch size
#                  earlier, so a repeat here is what moves them: argparse keeps
#                  the last value.
...
#
# ---- Seven values have to reach the trainer ----------------------------------
#
...
#
# The trainer's own command line names all seven. So this script starts the
...
if [ "$timed_out" -eq 1 ]; then
  ...
  echo "  seed and the align target are unchecked. Every arm of this card" >&2
  echo "  writes the same file names, so the leg is stopped rather than" >&2
  echo "  left to climb. Raise CF412_CHECK_TIMEOUT for a slower box, and" >&2
  echo "  read $TLOG" >&2
  exit 3
fi
...
if [ "$got_arch" != "$want_arch" ] || [ "$got_ema" != "$want_ema" ] \
  ...
  log "arm $ARM STOPPED — trained arch '$got_arch' / k $got_k /" \
      ...
      "batch $got_batch / align $got_align"
  echo "ABORT: this leg's trainer runs the shape '$got_arch' at depth" >&2
  echo "  '$got_k' under '$got_red', momentum '$got_ema', decay" >&2
  echo "  '$got_decay', seed '$got_seed', batch '$got_batch' and align" >&2
  echo "  target '$got_align'. Arm '$ARM' is '$want_arch' at depth" >&2
  echo "  '$ARM_K' under '$ARM_REDUCE', momentum '$want_ema', decay" >&2
  echo "  '$want_decay', seed '$ARM_SEED', batch '$CF412_BATCH_SIZE' and" >&2
  echo "  align target 'teacher'. The shape reads <d_model> <num_layers>" >&2
...
log "arm $ARM arch=$got_arch k=$got_k reduce=$got_red ema='$got_ema'" \
    "decay='$got_decay' seed=$got_seed batch=$got_batch align=$got_align" \
    "OK — all seven reached the trainer"
