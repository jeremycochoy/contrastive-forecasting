...
|----------|-----------------------------|-------|-----------------------------|-------|
| ncpc     | 3/6 (arm 5/6_v2/bimoco)     | 0.281 | 5/6 (all except arm 6_v2)   | 0.047 (one-sided, N=6) |
...
**Caveat on the error bars.** The ±0.01 shown is not a measured seed-replicate CI for this experiment (each cell is `N=1`). It is a borrowed constant from the 2026-05-08 τ-sweep paired reruns via [LeJEPA-SIGReg-τ report annex F](../2026-06-21_lejepa_sigreg_tau098/lejepa_sigreg_tau098.md#f-seed-noise-band), on a different backbone and a different recipe. It is here as a visual reference for "differences smaller than this in the past turned out to be within-seed noise", not as a confidence interval for this ranking. The top-vs-#2 gap is 12× that borrowed band, which is why it is called out separately.
...
![u_batchtime per arm, one panel per arm × variant, `h_t` solid + `e_t` dashed, x-axis log training step. base panels: `u_batchtime(h_t)` ≥ 0.97 for arm 3/4/5 and 0.5–0.8 for arm 6 v2 and bimoco. combab panels: `u_batchtime(h_t)` on arm 6 v2 and bimoco is lower than in their base panels.](plots/dim_usage_per_arm.png)
