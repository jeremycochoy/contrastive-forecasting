...
### GM table (head-matched, full-97)

| head / ckpt | `λ_e=1.0, λ_h=0.1` (anchor) | arm 1 (10.0, 0.1) | arm 2 (10.0, 1.0) | arm 3 (10.0, 10.0) | arm 5 (100.0, 0.1) |
| --- | ---: | ---: | ---: | ---: | ---: |
| 2L / best | 1.1470 | 1.1474 | **1.1302** | 1.1540 | 1.1554 |
| 2L / last | 1.1681 | **1.1610** | 1.1756 | 1.1682 | 1.1828 |
| 6L / best | 1.1408 | 1.1447 | **1.1294** | 1.1465 | 1.1462 |
| 6L / last | 1.1482 | **1.1473** | 1.1515 | 1.1538 | 1.1682 |

### Δ vs `λ_e=1.0, λ_h=0.1` with paired-bootstrap 95% CI (B=10 000, n=97 configs)
...
Bold = the one cell whose 95% CI excludes zero. Δ is on the absolute GM-Rel MASE scale via `GM_anchor · (exp(quantile) − 1)`.
...
![Total training loss (50-step rolling mean) over 12 500 steps for the 4 sweep arms and the 2 prior λ_h=0.1 anchors.](plots/loss_curve.png)

![L_SIGReg(e_t), L_SIGReg(h_t), u_batch(e_t), u_temporal(e_t) over 12 500 steps. Top row uses log y; the `1/K ≈ 0.00260` dotted line marks the one-direction floor of the dim-usage metrics.](plots/sigreg_e_inspection.png)

![Cross-batch and cross-time dim-usage of `h_t` (solid) and `e_t` (dashed) for the sweep arms and the 2 anchors; clipped to `[1/K, 1]`.](plots/uniformity.png)
...
| label | recipe |
| --- | --- |
| `enc3+CPC, B=1024` | non-SIGReg, non-EMA baseline |
| `EMA enc3+CPC, B=1024` | EMA-target only, no SIGReg |
| `SIGReg λ_e=λ_h=0.1` | `λ_e=λ_h=0.1`, B=512 (per-config rel-MASE re-read from `reports/2026-06-20_lejepa_sigreg/`) |
| `SIGReg λ_e=1.0, λ_h=0.1` | the report's bootstrap baseline (per-config rel-MASE re-read from `reports/2026-06-22_lejepa_sigreg_emb10/`) |
...
| **best-ckpt / last-ckpt** | `best` = backbone checkpoint at lowest train-loss step; `last` = backbone at step 12 500. The q-head is trained from each backbone separately. |
...
- **Plot script.** [`scripts/build_plots.py`](scripts/build_plots.py).
