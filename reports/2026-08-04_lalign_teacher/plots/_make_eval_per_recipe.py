...
Companion to the combined chart in _make_eval_progression.py: same data, same
y-range, split by recipe so each recipe's five settings can be read without
tracing a line across 30 others.

All 30 cells have backbone 40k and 100k; the ones that improved over that
stretch were extended to 200k. Splitting by recipe keeps every label beside its
own line inside its own panel.

Marks follow the rest of the report: `⟲` on the two recipes retrained with
`--align-target teacher`, `‡` on the one backbone whose loss rose over the whole
0-40k window.
"""
...
YMIN, YMAX = 1.13, 1.82  # same range as _make_eval_progression.py
RETRAINED_MARK = {"arm5": " ⟲", "arm6_v2": " ⟲"}


def rose_over_0_40k():
    """{(arm, var)} whose backbone loss ended the 0-40k window above its start.

    Same `results/anomaly_windows.csv` the bar charts and the tables read.
    """
    import csv
    slugs = {f"{a}{v}": (a, v) for a in C.ARMS for v in C.VARIANTS}
    out = set()
    with open(HERE.parent / "results" / "anomaly_windows.csv", newline="") as fh:
        for r in csv.DictReader(fh):
            if r["window"] != "0k-40k" or float(r["delta"]) <= 0:
                continue
            rest = r["run"].removeprefix("bb_small_")
            hit = [g for g in slugs if rest.startswith(g + "_") or rest == g]
            if hit:
                out.add(slugs[max(hit, key=len)])
    return out
...
for ax, arm in zip(axes.flat, C.ARMS):
    ...
    for (var, vals, last_x, last_v), y_lab in zip(drawn, ys):
        ...
        mark = " ‡" if (arm, var) in ROSE else ""
        ax.text(2.33, y_lab, f"{C.VAR_SHORT[var]}{mark} {have[-1]:.3f}{delta} {tag}",
                color=colour, fontsize=8, va="center", ha="left")

    ax.set_title(f"{arm}{RETRAINED_MARK.get(arm, '')}   {C.ARM_LOSS[arm]}",
...
handles += [Line2D([], [], color="#d8d6cf", lw=1.4, label="the other five recipes"),
            Line2D([], [], color="#c04040", ls="--", lw=1.2,
                   label="seasonal-naive parity (MASE=1), below the drawn range"),
            Line2D([], [], ls="none", marker="none",
                   label="⟲ recipe retrained with --align-target teacher"),
            Line2D([], [], ls="none", marker="none",
