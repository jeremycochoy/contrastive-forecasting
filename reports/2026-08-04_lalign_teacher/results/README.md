...
| `latent_drift_setting_vs_base.csv` | per setting: how many of the six loss recipes move `h_t` (and `e_t`) less than their own base setting, which recipes those are, and the two-sided exact binomial p on the count. Written from `latent_movement_pairs.csv` by `scripts/make_report_tables.py`, so the report's latent-drift table is rebuildable. |
