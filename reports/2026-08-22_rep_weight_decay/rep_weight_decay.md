...
A linear decay of the L_rep weight, the contrastive representation term of the training loss, does not improve the GM-Relative MASE of the k = 32 cell, one backbone configuration at rollout depth 32. Its best arm, momentum 0.940 with a decay ramp of 2,000 steps, loses to the no-decay reference by 1.7 times the seed range.
...
|---|---|---|---|---|
| each scored arm against the reference 1.1491 | +0.0804 to +0.2132 | 1.7 to 4.5 | 0.0016 | threshold for `dec_m090r100_ramp2k`, `dec_m090r100_ramp1k` and `dec_m080_r200`, rank for the other ten |
...
- EMA: `--ema-tau <tau>`, with `--ema-tau-end 1.0 --ema-tau-ramp-steps <ramp>` on the ramped schedules. The momentum at the stop is the value the schedule holds at that step.
