...
#
#   results/scores.csv         one 97-config GM-Relative MASE per (ARM, STOP).
#                              The arm is the EMA schedule, the decay ramp and
#                              the seed, so `ema_tau`, `ema_end`, `ema_ramp`,
#                              `ema_at_stop`, `ramp` and `seed` all ride beside
#                              the score. Rows that share a schedule and a ramp
#                              and differ in the seed are that treatment's
#                              spread, not two treatments.
...
# and the tag carries all three. So the table is read back out of the file
# names, and out of the eval log behind any name this results/ lost.
#
...
# ---- The scores --------------------------------------------------------------
#
# THE KEY IS (ARM, STOP). An arm carried past 40,000 steps holds one measured
# score at each stop, and a table keyed by the arm alone keeps one of them and
# drops the other: `dec_m090r100_ramp1k` scored 1.2322 at 40,000 steps and
# 1.2381 at 80,000.
#
...
# One row, filed under its (arm, stop). The FIRST source to reach a key keeps
# it, so no source can overwrite the row of another stop.
score_row(){  # <arm> <stop steps> <head steps> <encoder> <score>
  local key="$1 $2" ema_tau ema_end ema_ramp
  [ -n "${ROWS[$key]:-}" ] && return 0
  read -r ema_tau ema_end ema_ramp <<<"$(cf409_ema_sig "$1")"
  ROWS[$key]="$1,$ema_tau,$ema_end,$ema_ramp,$(cf409_momentum_at "$1" "$2"),$(cf409_seed "$1"),$CF409_REP_W_END,$(cf409_decay_ramp_of "$1"),$(cf409_rep_w_at "$1" "$2"),$CF409_ALIGN_TARGET,$2,$3,$4,$5"
}

# Source 1: the score files under this results/. The tag carries the stop and
# the head budget, so a stop that `CF409_STOPS` does not name still lands.
for f in "$CF409_RESULTS"/score_*.txt; do
  ...
  cf409_is_in "$arm" "$CF409_ARMS" || {
    ...
    continue; }
  score_row "$arm" "$(cf409_steps_of "$stop_l")" \
    "$(cf409_steps_of "$head_l")" "$enc" "$score"
done

# Source 2: the eval's own log, for a pair whose score file this results/ no
# longer holds. `cf409_score_of` states the order of the two.
while read -r arm stop; do
  [ -n "${ROWS["$arm $stop"]:-}" ] && continue
  score="$(cf409_score_of "$arm" "$stop")" || continue
  score_row "$arm" "$stop" "$CF409_HEAD_STEPS" "$CF409_ENC" "$score"
done < <(cf409_pairs)

{
  echo "arm,ema_tau,ema_end,ema_ramp,ema_at_stop,seed,rep_end,ramp,rep_w_at_stop,align_target,stop,head_steps,encoder,score"
  # By arm, then by stop, so two stops of one arm read as one pair of lines.
  [ "${#ROWS[@]}" -eq 0 ] || printf '%s\n' "${ROWS[@]}" \
    | sort -t, -k1,1 -k11,11n
} >"$OUT.$TMP"
mv -f "$OUT.$TMP" "$OUT"
