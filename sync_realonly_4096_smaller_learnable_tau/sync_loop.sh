#!/bin/bash
# Atomic sync loop for one arm of exp_realonly_4096_smaller_learnable_tau.
# Mirrors sync_compositesynth_v5envboost/sync_loop.sh with run-name pattern
# tiny_realonly_4096_<arm> and exp_realonly_4096_smaller_learnable_tau experiment dir.
...
REMOTE_RESULTS_DIR="/workspace/app/experiments/exp_realonly_4096_smaller_learnable_tau/results"
REMOTE_LOG="/workspace/app/run_learnable_tau.log"
LOCAL_BASE="/Users/jeremycochoy/Desktop/workspace/trading/contrastive-forecasting/sync_realonly_4096_smaller_learnable_tau/learnable"
...
echo "[$(date)] Sync loop start arm=${ARM} realonly_4096_smaller_learnable_tau instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
...
HEAD="R1q_realonly_4096_smaller_learnable_tau"

cycle_count=0
while true; do
    cycle_count=$((cycle_count + 1))
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === arm=${ARM} realonly_4096_smaller_learnable_tau cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    atomic_scp "${REMOTE_RESULTS_DIR}/gift_eval_learnable/all_results.csv" \
               "${LOCAL_RESULTS}/all_results.csv" "${CSV_MIN}" || true
    atomic_scp "${REMOTE_RESULTS_DIR}/gift_eval_learnable/summary.txt" \
    ...
    if grep -q "run_smaller_learnable_tau: ALL DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
