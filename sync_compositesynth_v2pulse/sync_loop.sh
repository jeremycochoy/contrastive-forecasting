#!/bin/bash
# Atomic sync loop for one arm of exp_compositesynth_v2pulse_2arm.
# Usage: ./sync_loop.sh <SSH_HOST> <SSH_PORT> <INSTANCE_ID> <ARM>
# ARM ∈ {revin, ewma128}. Two copies run concurrently — one per instance.
...
REMOTE_RESULTS_DIR="/workspace/app/experiments/exp_compositesynth_v2pulse_2arm/results"
REMOTE_LOG="/workspace/app/run_${ARM}.log"
LOCAL_BASE="/Users/jeremycochoy/Desktop/workspace/trading/contrastive-forecasting/sync_compositesynth_v2pulse/${ARM}"
...
echo "[$(date)] Sync loop start arm=${ARM} instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
...
BB="tiny_compsyn_v2pulse_${ARM}"
HEAD="R1q_compsyn_v2pulse_${ARM}"

cycle_count=0
while true; do
    cycle_count=$((cycle_count + 1))
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === arm=${ARM} cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    if grep -q "ARM ${ARM} ALL DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  === ARM ${ARM} ALL DONE detected; final sync ===" | tee -a "${LOG_FILE}"
        atomic_scp "${REMOTE_RESULTS_DIR}/gift_eval_${ARM}/all_results.csv" \
                   "${LOCAL_RESULTS}/all_results.csv" "${CSV_MIN}" || true
        atomic_scp "${REMOTE_RESULTS_DIR}/gift_eval_${ARM}/summary.txt" \
                   "${LOCAL_RESULTS}/summary.txt" "${CSV_MIN}" || true
        atomic_scp "${REMOTE_LOG}" "${LOCAL_BASE}/run.log" "${LOG_MIN}" || true
        echo "[$(date)] Sync loop arm=${ARM} finished" | tee -a "${LOG_FILE}"
        exit 0
    fi
