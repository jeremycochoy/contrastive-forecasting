#!/bin/bash
# exp_realonly_4096_smaller_learnable_tau (#28) — CLIP-style learnable τ.
#
# Adds `log_inv_tau` as a single trainable scalar on the model. Loss uses
# τ = exp(-log_inv_tau). After each optimizer.step we clamp log_inv_tau to
# [0, log(100)] so τ ∈ [0.01, 1.0]. Init: log(1/0.07) ≈ 2.659 (τ=0.07).
#
# Per user (May 2): NOT gated on #27's τ-sweep winner. Init at τ=0.07 (the
# established baseline). 30k steps. bs=96.
#
# Usage:
#   bash experiments/exp_realonly_4096_smaller_learnable_tau/run.sh
set -e
cd /workspace/app

LOG="/workspace/app/run_learnable_tau.log"
exec >> >(tee -a "$LOG") 2>&1
echo "" && echo "=== run_smaller_learnable_tau: starting ===" && date
...
BB="tiny_realonly_4096_smaller_learnable_tau"
QH="R1q_realonly_4096_smaller_learnable_tau"
RES_DIR="experiments/exp_realonly_4096_smaller_learnable_tau/results/gift_eval"
mkdir -p "$RES_DIR"

# bs=96 chosen via local benchmark on smaller arch + T=4096 + cross-batch sims
echo "" && echo "=== STAGE B: $BB (smaller, EWMA-128, learnable τ init=0.07, bs=96) ===" && date
...
# Head training: backbone frozen, head doesn't use τ. Auto-detect of
# learnable_tau in checkpoint loads log_inv_tau cleanly without side effects.
...
echo "" && echo "=== run_smaller_learnable_tau: ALL DONE ===" && date
echo ""
