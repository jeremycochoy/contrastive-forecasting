#!/bin/bash
# #350 — downstream for one backbone (arm: bilinear|cpc), head_layers 2|6:
# freeze the backbone, train a fresh quantile forecasting head (best-loss
# backbone, 30k steps; then last backbone, resumed from best, re-adapted 10k),
# and score GIFT-Eval full-97. EXACT #348/#344 head + eval protocol and
# hyperparameters (head loads the backbone with main_w.*/cpc_w1.* stripped).
#   downstream.sh <arm: bilinear|cpc> <head_layers: 2|6> <gpu>
set -uo pipefail
...
log "downstream complete ($ARM ${HL}L)"
