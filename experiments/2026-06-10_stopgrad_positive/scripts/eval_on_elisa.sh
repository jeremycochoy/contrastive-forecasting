...
out="$RES/gift_eval_full_${OUT_TAG}_${HL}L"
...
python3 -u "$QEVAL" --resume --backbone-path "$BB" --head-path "$HEAD" --output-dir "$out" --strategy B4 \
  ...
  >>"$RES/run_eval_full_${OUT_TAG}_${HL}L.log" 2>&1 || { echo "EVAL $OUT_TAG ${HL}L FAILED"; exit 1; }
