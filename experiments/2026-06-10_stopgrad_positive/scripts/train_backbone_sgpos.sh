#!/bin/bash
# Stop-grad follow-up (#336) — backbone: the EXACT #328 best-arm recipe
# (allt·0.8% + explicit (A_norm,B_norm,C) crossfade triplet, NO forecaster
# bottleneck, 3-layer encoder, --subtract-contrastive-floor kept; same seed,
# batch, steps) with a SINGLE change: --stopgrad-positive-h — the encoder
# side h_{t+1} of the InfoNCE positive is stop-gradiented (numerator and
# denominator; SimSiam/BYOL-style target stop-grad).
# Single GPU, GLOBAL BATCH 1024.
#
#   train_backbone_sgpos.sh <gpu> [steps] [save_every]
set -uo pipefail
GPU="${1:?gpu}"; STEPS="${2:-12500}"; SAVE_EVERY="${3:-2500}"
SEED=20260520
WT="${WT:-/tmp/cf-sgpos}"
OUT="${OUT:-/home/jupyter/workspaces/contrastive-forecasting/experiments/2026-06-10_stopgrad_positive}"
NAME="bb_allt08_xftrip_nobn_enc3_sgpos_qk_aon_b1024"
...
# chunks + gradient-checkpoints the GRU). Same settings as #328's backbones.
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [bb-sgpos g$GPU] $*"; }
