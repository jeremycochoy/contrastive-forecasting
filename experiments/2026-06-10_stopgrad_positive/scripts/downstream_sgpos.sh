#!/bin/bash
# Stop-grad follow-up (#336) — downstream for the full-width sgpos backbone:
# trains the best-loss q-head + GIFT-Eval full-97, then the full-training
# (last) q-head (resumed from best, re-adapted) + eval. Same protocol and
# hyperparameters as #328's downstream_generic.sh (the reference arm's evals).
#   downstream_sgpos.sh <tag> <head_layers> <gpu>
#     backbone best = bb_<tag>_FINAL.pth ; last = bb_<tag>_final.pth
set -uo pipefail
TAG="${1:?tag}"; HL="${2:?head_layers}"; GPU="${3:?gpu}"
WT="${WT:-/tmp/cf-sgpos}"
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [dl-sgpos ${HL}L g$GPU] $*"; }
...
log "sgpos downstream complete (${HL}L)"
