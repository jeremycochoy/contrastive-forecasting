#!/bin/bash
# #374 arm bimoco (CORRECT loss) — split shape with MoCo on BOTH L_pred AND
# L_rep, where L_rep_moco is a proper normalized InfoNCE with the same-batch
# same-time <h_student, h_teacher> positive in the denominator (not the
# pooled-LSE-with-teacher-keys-no-positive of the earlier wrong run).
#
# The earlier bimoco (runs_bimoco/, results_bimoco/) is retained as-is under
# its original directory names and marked wrong in the report. This run
# lands under runs_bimoco_v2/ and results_bimoco_v2/ so it does not shadow
# the wrong data.
#
# Runs one instance on one GPU. Backbone (12,500 steps) + downstream 2L on
# GPU 0 + 6L on GPU 1 (parallel) via the same protocol as the original
# elisa_bimoco_arm_bimoco_launch.sh: best-loss q-head 30k + last-checkpoint
# q-head 10k re-adapt, full-97 GIFT-Eval B4 per cell.
set -uo pipefail
WT="${WT:-/tmp/contrastive-forecasting-374}"
OUT="$WT/experiments/2026-07-10_split_pred_rep"
RUNS="$OUT/runs_bimoco_v2"; RES="$OUT/results_bimoco_v2"; mkdir -p "$RUNS" "$RES"
NAME="bb_split_pred_rep_bimoco_v2_xftrip_nobn_enc3_emateach_sigreg_qk_aon_b512_cpc_tau090"
TAG="${NAME#bb_}"
SEED=20260520
...
DL_LOG="$RES/dl_bimoco_v2.log"
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [elisa-bimoco-v2] $*" | tee -a "$DL_LOG"; }
...
if [ -f "$BB" ]; then
  ...
  log "BB START (bimoco CORRECT: split + moco-negatives + moco-rep-keys (with positive)) gpu=$BB_GPU steps=$STEPS bs=512 ${RESUME}"
...
wait $pid2; rc2=$?
wait $pid6; rc6=$?
log "arm bimoco (correct) complete: 2L failed-cells=$rc2  6L failed-cells=$rc6"
exit $(( rc2 != 0 || rc6 != 0 ))
