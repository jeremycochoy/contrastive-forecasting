#!/bin/bash
# #374 — downstream q-head + GIFT-Eval for the split_pred_rep arm.
# Same head-training + eval protocol as the #366 arm C downstream
# (experiments/2026-06-28_sigreg_lambda_tau_cross/scripts/downstream_sigreg.sh):
# best-loss q-head (30k) + last-checkpoint q-head (resumed, 10k re-adapt),
# per head_layers ∈ {2, 6}, then full-97 GIFT-Eval B4 per cell.
#
#   downstream_split_pred_rep.sh <head_layers: 2|6> <gpu>
set -uo pipefail
HL="${1:?head_layers}"; GPU="${2:?gpu}"
TAG="split_pred_rep_xftrip_nobn_enc3_emateach_sigreg_qk_aon_b512_cpc_tau090"
: "${WT:?WT (worktree root containing train.py + experiments/hf_token.txt) must be set}"
: "${OUT:?OUT (per-experiment output dir for runs/ and results/) must be set}"
[ -d "$WT" ] || { echo "[dl-split ${HL}L] ABORT: WT does not exist: $WT" >&2; exit 2; }
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [dl-split ${HL}L g$GPU] $*"; }
...
if [ ! -f "$BBLAST" ]; then
  ...
  log "    means BB did not complete. The last-cell read depends on this."
  exit 1
fi
run_cell "qhead_${HL}L_${TAG}_last" "$BBLAST" "$RUNS/qhead_${HL}L_${TAG}_FINAL.pth" "10000" "1000" "${TAG}_last"
log "split_pred_rep downstream complete (${HL}L): failed cells=$fail"
exit "$fail"
