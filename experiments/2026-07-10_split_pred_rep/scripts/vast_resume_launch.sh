#!/bin/bash
# Launch the split_pred_rep backbone training on a vast.ai instance,
# resuming from the transferred best_gap.pth. Identical model spec and
# CLI to train_backbone_split_pred_rep.sh; only difference is the
# --resume flag and the path layout under /workspace/cf-374.
#
#   vast_resume_launch.sh [steps] [save_every]
set -uo pipefail
STEPS="${1:-12500}"
SAVE_EVERY="${2:-2500}"
SEED=20260520
...
RESUME_PATH="$RUNS/${NAME}_best_gap.pth"
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [vast-split-pred-rep] $*"; }
[ -n "$HF_TOKEN" ] || { log "ABORT: empty HF_TOKEN — HF stream would throttle GPU"; exit 1; }
[ -f "$BB" ] && { log "BB SKIP ($NAME FINAL exists)"; exit 0; }
[ -f "$RESUME_PATH" ] || { log "ABORT: resume checkpoint not found at $RESUME_PATH"; exit 1; }
tlog="$RES/run_${NAME}.log"
log "RESUME from $(basename "$RESUME_PATH")"
log "BB START arm=split-pred-rep bs=512 steps=$STEPS xshh_chunk=$XSHH_ALLT_CHUNK cpc_chunk=$CPC_CB_CHUNK"
python3 -u "$TRAIN" --resume "$RESUME_PATH" --qk-norm --attn-out-norm \
