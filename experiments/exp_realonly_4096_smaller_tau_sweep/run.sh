#!/bin/bash
# exp_realonly_4096_smaller_tau_sweep — τ sweep on smaller-EWMA-128 with
...
#
# Usage:
#   bash experiments/exp_realonly_4096_smaller_tau_sweep/run.sh 005    # τ=0.05
#   bash experiments/exp_realonly_4096_smaller_tau_sweep/run.sh 020    # τ=0.20
set -e
cd /workspace/app

ARM="${1:?usage: run.sh <005|020>}"
case "$ARM" in
    005) TAU="0.05" ;;
    020) TAU="0.20" ;;
    *) echo "Unknown arm: $ARM (expected 005 or 020)" >&2; exit 1 ;;
esac
