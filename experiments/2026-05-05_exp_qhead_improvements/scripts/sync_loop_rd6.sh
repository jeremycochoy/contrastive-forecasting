...
REMOTE_LOG="/workspace/app/run_qhead_beta_rd6.log"
LOCAL_BASE="/home/jupyter/contrastive-forecasting/sync_qhead_beta_rd6"
...
mkdir -p "${LOCAL_CKPT}"
echo "[$(date)] Sync loop start qhead_beta_rd6 instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
...
HEAD_MIN=20000000
HEAD_OPT_MIN=40000000
CSV_MIN=1
LOG_MIN=1

E4="R6_E8_xfmr6L_bidir_fl128_quant_moirai_cosine"

cycle_count=0
while true; do
    cycle_count=$((cycle_count + 1))
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === qhead_beta_rd6 cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    if grep -q "R6 ALL DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  === R6 ALL DONE detected; final sync ===" | tee -a "${LOG_FILE}"
        atomic_scp "${REMOTE_LOG}" "${LOCAL_BASE}/run.log" "${LOG_MIN}" || true
        echo "[$(date)] Sync loop qhead_beta_rd6 finished" | tee -a "${LOG_FILE}"
        exit 0
    fi
