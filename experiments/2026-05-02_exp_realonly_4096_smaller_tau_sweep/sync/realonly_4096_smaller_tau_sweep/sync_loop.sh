...
echo "[$(date)] Sync loop start arm=${ARM} realonly_4096_smaller_tau instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
...
HEAD="R1q_realonly_4096_smaller_tau${ARM}"

cycle_count=0
while true; do
    cycle_count=$((cycle_count + 1))
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === arm=${ARM} realonly_4096_smaller_tau cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    atomic_scp "${REMOTE_RESULTS_DIR}/gift_eval_tau${ARM}/all_results.csv" \
               "${LOCAL_RESULTS}/all_results.csv" "${CSV_MIN}" || true
    atomic_scp "${REMOTE_RESULTS_DIR}/gift_eval_tau${ARM}/summary.txt" \
    ...
    if grep -q "ARM ${ARM} ALL DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  === ARM ${ARM} tau${ARM} ALL DONE detected; final sync ===" | tee -a "${LOG_FILE}"
        atomic_scp "${REMOTE_RESULTS_DIR}/gift_eval_tau${ARM}/all_results.csv" \
                   "${LOCAL_RESULTS}/all_results.csv" "${CSV_MIN}" || true
        atomic_scp "${REMOTE_RESULTS_DIR}/gift_eval_tau${ARM}/summary.txt" \
                   "${LOCAL_RESULTS}/summary.txt" "${CSV_MIN}" || true
        atomic_scp "${REMOTE_LOG}" "${LOCAL_BASE}/run.log" "${LOG_MIN}" || true
        echo "[$(date)] Sync loop arm=${ARM} tau_sweep finished" | tee -a "${LOG_FILE}"
        exit 0
    fi
