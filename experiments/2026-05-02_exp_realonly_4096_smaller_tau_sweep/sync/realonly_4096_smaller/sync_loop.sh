#!/bin/bash
# Atomic sync loop for one arm of exp_realonly_4096_smaller_2arm.
# Mirrors sync_compositesynth_v5envboost/sync_loop.sh with run-name pattern
# tiny_realonly_4096_<arm> and exp_realonly_4096_smaller_2arm experiment dir.
...
REMOTE_RESULTS_DIR="/workspace/app/experiments/exp_realonly_4096_smaller_2arm/results"
REMOTE_LOG="/workspace/app/run_smaller_${ARM}.log"
LOCAL_BASE="/Users/jeremycochoy/Desktop/workspace/trading/contrastive-forecasting/sync_realonly_4096_smaller/${ARM}"
...
echo "[$(date)] Sync loop start arm=${ARM} realonly_4096_smaller instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
...
HEAD="R1q_realonly_4096_smaller_${ARM}"

cycle_count=0
while true; do
    cycle_count=$((cycle_count + 1))
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === arm=${ARM} realonly_4096_smaller cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    if grep -q "ARM ${ARM} ALL DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  === ARM ${ARM} realonly_4096_smaller ALL DONE detected; final sync ===" | tee -a "${LOG_FILE}"
        atomic_scp "${REMOTE_RESULTS_DIR}/gift_eval_${ARM}/all_results.csv" \
                   "${LOCAL_RESULTS}/all_results.csv" "${CSV_MIN}" || true
        atomic_scp "${REMOTE_RESULTS_DIR}/gift_eval_${ARM}/summary.txt" \
                   "${LOCAL_RESULTS}/summary.txt" "${CSV_MIN}" || true
        atomic_scp "${REMOTE_LOG}" "${LOCAL_BASE}/run.log" "${LOG_MIN}" || true
        echo "[$(date)] Sync loop arm=${ARM} realonly_4096_smaller finished" | tee -a "${LOG_FILE}"
        exit 0
    fi
