#!/bin/bash
# #344 — downstream for one CPC-InfoNCE backbone: trains the best-loss q-head
# (30k steps) and, when the last (12.5k) backbone exists, the full-training
# q-head (resumed from best, re-adapted 10k), for head_layers 2 or 6 — the
# EXACT #339/#341 protocol and hyperparameters. With DO_EVAL=1 also runs the
# serial full-97 GIFT-Eval per cell. Backbone encoder depth is auto-detected
# from the checkpoint; both #344 arms are full-width (no forecaster bottleneck).
#   downstream_cpc.sh <arm: enc3|enc6> <head_layers: 2|6> <gpu>
set -uo pipefail
ARM="${1:?arm}"; HL="${2:?head_layers}"; GPU="${3:?gpu}"
# TAG_OVERRIDE lets non-standard arms (e.g. the cpc+align/no-main arm, which
# has no _sgpos_ in its name) reuse this downstream unchanged.
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [dl-cpc-$ARM ${HL}L g$GPU] $*"; }
gm(){ grep 'Aggregate GM-Relative MASE' "$1" 2>/dev/null | grep -oE '[0-9]+\.[0-9]+$' | tail -1; }
[ -f "$BB" ] || { log "ABORT backbone missing: $BB"; exit 1; }
# Both #344 arms are full-width (no bottleneck); encoder depth auto-detected.
FCST=()
...
# last (full-training): resume best head onto final.pth (the FINAL copy has no
# _optimizer.pth companion ⇒ weights-only load, full 10k re-adapt steps).
if [ -f "$BBLAST" ]; then
...
log "cpc downstream complete ($ARM ${HL}L)"
