#!/bin/bash
# #357 free-arm — downstream q-head + GIFT-Eval for the no-EMA SIGReg-B=512
# enc3+CPC backbone. Same head-matched protocol as the τ=0.98 arm and #355.
#
#   downstream_sigreg_noema.sh <head_layers: 2|6> <gpu>
set -uo pipefail
HL="${1:?head_layers}"; GPU="${2:?gpu}"
TAG="allt08_xftrip_nobn_enc3_sigreg_qk_aon_b512_cpc_noema"
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [dl-sigreg-noema ${HL}L g$GPU] $*"; }
...
log "sigreg-noema downstream complete (${HL}L)"
