...
# with --ema-tau 0.9 (half-life ln(0.5)/ln(0.9) ≈ 6.6 steps).
# Same recipe as the τ=0.98 arm with one flag change: --ema-tau 0.98 -> 0.9.
#
#   train_backbone_sigreg_tau090.sh <gpu> [steps] [save_every]
set -uo pipefail
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [bb-sigreg-tau090-enc3 g$GPU] $*"; }
...
log "BB START arm=sigreg-tau090-enc3 bs=512 steps=$STEPS xshh_chunk=$XSHH_ALLT_CHUNK cpc_chunk=$CPC_CB_CHUNK ${RESUME}"
