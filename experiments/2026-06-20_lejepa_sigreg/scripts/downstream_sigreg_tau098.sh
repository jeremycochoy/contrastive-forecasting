#!/bin/bash
# #357 — downstream q-head + GIFT-Eval for the SIGReg-B=512 enc3+CPC backbone
# with --ema-tau 0.98. Identical protocol to #355's downstream_sigreg.sh
# (which targeted --ema-tau 0.99); the comparison must be head-matched
# against #355, #353, #344.
# Trains best-loss q-head (30k) then last-checkpoint q-head (resumed, 10k
# re-adapt), per head_layers, then runs full-97 GIFT-Eval for each cell.
#
#   downstream_sigreg_tau098.sh <head_layers: 2|6> <gpu>
set -uo pipefail
HL="${1:?head_layers}"; GPU="${2:?gpu}"
TAG="allt08_xftrip_nobn_enc3_emateach_sigreg_qk_aon_b512_cpc_tau098"
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [dl-sigreg-tau098 ${HL}L g$GPU] $*"; }
...
log "sigreg-tau098 downstream complete (${HL}L)"
