...
# WITHOUT the EMA-teacher mechanism. Same recipe as #355 minus three flags:
#   --ema-embedding  REMOVED
#   --ema-encoder    REMOVED
#   --ema-tau ...    REMOVED
# Everything else stays the #355 SIGReg + B=512 enc3+CPC recipe verbatim:
# --sigreg-embedding, --sigreg-encoding, --cpc-infonce-weight 1.0,
# GRU patch-embed, 3L encoder, 6L decoder, dropkey 0.70, mix_ratio 0.0078125,
# 12,500 steps, same dtypes/seed/dataset.
#
#   train_backbone_sigreg_noema.sh <gpu> [steps] [save_every]
set -uo pipefail
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [bb-sigreg-noema-enc3 g$GPU] $*"; }
...
log "BB START arm=sigreg-noema-enc3 bs=512 steps=$STEPS xshh_chunk=$XSHH_ALLT_CHUNK cpc_chunk=$CPC_CB_CHUNK ${RESUME}"
