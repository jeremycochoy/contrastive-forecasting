#!/bin/bash
# #357 free-arm — downstream q-head + GIFT-Eval for the τ=0.8 EMA arm.
#
#   downstream_sigreg_tau080.sh <head_layers: 2|6> <gpu>
set -uo pipefail
HL="${1:?head_layers}"; GPU="${2:?gpu}"
TAG="allt08_xftrip_nobn_enc3_emateach_sigreg_qk_aon_b512_cpc_tau080"
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [dl-sigreg-tau080 ${HL}L g$GPU] $*"; }
...
log "sigreg-tau080 downstream complete (${HL}L)"
