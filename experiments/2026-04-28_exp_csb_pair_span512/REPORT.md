...
1. **The span=512 baseline (no_time_neg) checkpoint was lost.** During
   the wrap-up of `2026-04-27_exp_csb_synth`, an auxiliary git worktree was torn
...
2. **`2026-04-27_exp_csb_synth` had a multi-resume confound.** That CSB run
...
|---|---:|---:|---|---|---|
| `fe+mu @ 30k span=512` (lost baseline, 2026-04-27_exp_span_sweep_synth) | 0.848 | 0.413 | `_best_gap` | clean | — |
| `fe+mu @ 30k span=512 +cosine_similarity_batch` (2026-04-27_exp_csb_synth) | 0.886 | 0.434 | `_best_loss` | multi-resume | 8k → 24k → 30k |
...
- **B vs 2026-04-27_exp_csb_synth (same loss + same selector, different
  continuity):** 0.883 vs 0.886, within noise. The multi-resume runs in
  2026-04-27_exp_csb_synth turned out to be approximately a clean run for the CSB
  arm.
...
The original `2026-04-27_exp_csb_synth` finding ("CSB is 4.5% worse than baseline")
