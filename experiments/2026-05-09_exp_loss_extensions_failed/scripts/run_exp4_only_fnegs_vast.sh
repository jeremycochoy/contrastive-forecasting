#!/bin/bash
# Exp 4 STANDALONE (non-cumulative) — f-side cross-(b,c) negatives ONLY,
# no inheritance of Exp 3's (h_t, f_t) positive (which is showing
# AUC ≈ 0.51 stuck at chance on elisa). Tests whether the f-cross-bc
# negative term is useful on its own, before declaring it broken-with-Exp 3.
#
# Loss: cosine_similarity_batch_add_f_cross_negs (PR #182).
# Architecture: gru @ τ=0.20, 15k steps fresh from scratch (same recipe
# as Exp 3 / the cumulative Exp 4 launcher except --loss-shape changes).
#
# Speculative vast.ai 5090 spot run. Adapted verbatim from
# experiments/2026-05-09_exp_loss_extensions/scripts/run_exp4_fnegs_vast.sh
# (CUDA cu128 / driver 575 fix inherited).
#
# Usage (on the vast container):
#   nohup bash experiments/2026-05-09_exp_loss_extensions/scripts/run_exp4_only_fnegs_vast.sh \
#       > /workspace/app/run_exp4_only_fnegs.log 2>&1 &
#   disown
...
HF_PATH="small_v1"
LOSS="cosine_similarity_batch_add_f_cross_negs"
SAVE_DIR="checkpoints"
TAU="0.20"
NAME="exp4_only_fnegs_tau_0_20"
MAX_RETRIES=5
RETRY_SLEEP=60
...
if [ -f "${SAVE_DIR}/${NAME}_FINAL.pth" ]; then
    echo "=== SKIP exp4 only-fnegs (${SAVE_DIR}/${NAME}_FINAL.pth exists) ===" && date
    exit 0
fi

echo "=== EXP4 only-fnegs (gru, τ=${TAU}, 15k) → run_name=${NAME} ===" && date
echo "=== loss=${LOSS} (NON-cumulative — no (h_t,f_t) positive) ==="
...
if [ "$rc" -ne 0 ]; then
    echo "=== EXP4-ONLY FAILED after ${MAX_RETRIES} attempts (rc=${rc}) ===" && date
    exit "$rc"
fi
...
if [ ! -f "${SAVE_DIR}/${NAME}_best_loss.pth" ]; then
    echo "=== EXP4-ONLY ERROR — ${NAME}_best_loss.pth missing post-train ==="
    exit 1
fi
