#!/bin/bash
# Exp 5 — skip-step f-negatives: cos(f_{b,c,t}, f_{b,c,t+2}) added to the
# baseline cosine_similarity_batch negatives sum. NON-cumulative. Reformulates
# the user's original "f_t vs f_{t+1}" spec at skip=2 (the existing neg_zy term
# already covers f_t vs f_{t+1} same-channel for C=1).
#
# Loss: cosine_similarity_batch_add_skip_f_negs (PR #184).
# Architecture: gru @ τ=0.20, 15k steps fresh from scratch (same recipe as
# Exp 4-only / Exp 3 except --loss-shape changes).
#
# Speculative vast.ai 5090 spot run. Adapted verbatim from
# experiments/2026-05-09_exp_loss_extensions/scripts/run_exp4_only_fnegs_vast.sh
# (CUDA cu128 / driver 575 fix inherited).
#
# Usage (on the vast container):
#   nohup bash experiments/2026-05-09_exp_loss_extensions/scripts/run_exp5_skip_fnegs_vast.sh \
#       > /workspace/app/run_exp5_skip_fnegs.log 2>&1 &
#   disown
...
HF_PATH="small_v1"
LOSS="cosine_similarity_batch_add_skip_f_negs"
SAVE_DIR="checkpoints"
TAU="0.20"
NAME="exp5_skip_fnegs_tau_0_20"
MAX_RETRIES=5
RETRY_SLEEP=60
...
if [ -f "${SAVE_DIR}/${NAME}_FINAL.pth" ]; then
    echo "=== SKIP exp5 skip-fnegs (${SAVE_DIR}/${NAME}_FINAL.pth exists) ===" && date
    exit 0
fi

echo "=== EXP5 skip-fnegs (gru, τ=${TAU}, 15k) → run_name=${NAME} ===" && date
echo "=== loss=${LOSS} (NON-cumulative — adds cos(f_t, f_{t+2}) neg term) ==="
...
if [ "$rc" -ne 0 ]; then
    echo "=== EXP5 FAILED after ${MAX_RETRIES} attempts (rc=${rc}) ===" && date
    exit "$rc"
fi
...
if [ ! -f "${SAVE_DIR}/${NAME}_best_loss.pth" ]; then
    echo "=== EXP5 ERROR — ${NAME}_best_loss.pth missing post-train ==="
    exit 1
fi
