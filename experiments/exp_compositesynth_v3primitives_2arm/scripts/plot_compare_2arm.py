"""Comparison plot: composite-v3primitives (this exp) vs composite (phase-1)
vs periodic (phase-0 baseline), at matched norms (RevIN and EWMA-128).

Reads six CSVs:
  experiments/exp_compositesynth_v3primitives_2arm/results/gift_eval_{revin,ewma128}/all_results.csv  (this exp)
  experiments/exp_compositesynth_2arm/results/gift_eval_{revin,ewma128}/all_results.csv          (phase 1)
  experiments/exp_dualemb_3arm/results/gift_eval_{revin,ewma128}/all_results.csv                 (phase 0)

Produces a single PNG with four panels:
  - Aggregate (GM, median) MASE per arm — 6 bars
  - MASE CDF (6 lines)
  - Per-domain GM-MASE (6 bars/domain)
  - Head-to-head: pulse-EWMA-128 vs composite-EWMA-128 vs periodic-EWMA-128

Run from the repo root:
    python experiments/exp_compositesynth_v3primitives_2arm/scripts/plot_compare_2arm.py
"""
...
OUT = HERE / "plots" / "gift_eval_v3primitives_compare.png"
...
def main() -> None:
    arms = [
        ("v3primitives_revin",  RESULTS_DIR / "gift_eval_revin" / "all_results.csv",
            "v3primitives + RevIN",     "#8B0000"),
        ("v3primitives_ewma128", RESULTS_DIR / "gift_eval_ewma128" / "all_results.csv",
            "v3primitives + EWMA-128",  "#FF4500"),
    ...
    # Panel 4: focused head-to-head — at EWMA-128, the spike-deficit hypothesis test
    ax = axes[1, 1]
    pairs = []
    if "v3primitives_ewma128" in data and "comp_ewma128" in data:
        pairs.append(("v3primitives vs composite EWMA-128",
                      "v3primitives_ewma128", "comp_ewma128", "#FF4500"))
    if "v3primitives_ewma128" in data and "base_ewma128" in data:
        pairs.append(("v3primitives vs periodic EWMA-128",
                      "v3primitives_ewma128", "base_ewma128", "#2ca02c"))
    if pairs:
        lo, hi = 0.3, 200
        ...
        for label, comp_key, base_key, c in pairs:
            ...
            ax.scatter(xs, ys, s=14, alpha=0.7, color=c,
                       label=f"{label}: v3primitives wins {wins}/{len(xs)}")
        ax.set_xscale("log")
        ax.set_yscale("log")
        ax.set_xlim(lo, hi)
        ax.set_ylim(lo, hi)
        ax.set_xlabel("Reference (composite or periodic) MASE")
        ax.set_ylabel("v3primitives-EWMA-128 MASE")
        ax.set_title("v3primitives-EWMA-128 head-to-head")
        ax.grid(alpha=0.3)
    ...
    fig.suptitle(
        "Composite-v2-pulse vs composite-v1 vs periodic on GIFT-Eval — "
        f"{len(common)} configs, csb loss, mix=0.5, freq+seasonality emb",
        fontsize=11)
    ...
    for label, ck, bk, _ in pairs:
        ...
        print(f"  {label}: v3primitives-EWMA-128 beats reference on {(ys < xs).sum()}/{len(xs)}")
