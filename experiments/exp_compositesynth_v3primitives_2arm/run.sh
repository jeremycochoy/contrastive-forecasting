#!/bin/bash
# Single-arm composite-v3-primitives driver. Adds TRIANGLE + HALF_SIN
# primitives to the wave pool ({sin, square, saw, triangle, half_sin}).
# No pulse, no seas-heavy — pure primitive-variety test.
#
# Targets the diversity-vs-quantity insight from phase-2:
#   - v2pulse helped spike configs (-22% covid) but hurt strongly-periodic
#     configs (+30% solar/H, ett1/W) because adding pulse dilutes
#     sin/sq/saw rates from 1/3 → 1/4
#   - v2b (seas-heavy) regressed RevIN by adding redundant content rather
#     than diverse content
# Phase 3 hypothesis: more *distinct* primitives in the pool (5 instead
# of 3, all still periodic-shape-friendly) will broaden the contrastive
# signal without the pulse-dilution downside on periodic configs.
#
# Usage:
#   bash experiments/exp_compositesynth_v3primitives_2arm/run.sh revin
#   bash experiments/exp_compositesynth_v3primitives_2arm/run.sh ewma128
set -e
cd /workspace/app
...
exec >> >(tee -a "$LOG") 2>&1
echo "" && echo "=== run_compositesynth_v3primitives: ARM ${ARM} starting ===" && date

SETUP_MARKER="/workspace/app/.setup_done_compositesynth_v3"
...
BB="tiny_compsyn_v3_${ARM}"
QH="R1q_compsyn_v3_${ARM}"
RES_DIR="experiments/exp_compositesynth_v3primitives_2arm/results/gift_eval_${ARM}"
mkdir -p "$RES_DIR"
...
python3 -u experiments/freq-embedding/scripts/train.py \
    ...
    --mix-ratio 0.5 --synth-kind composite --more-primitives \
...
echo "" && echo "=== run_compositesynth_v3primitives: ARM ${ARM} ALL DONE ===" && date
echo ""
