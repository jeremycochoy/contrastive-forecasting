...
def parse_args():
    ...
    p.add_argument("--cpc-infonce-negs", choices=["matched", "all"], default="matched",
                   help="CPC InfoNCE negative set (#348). 'matched' (default, "
                        "the #344 term): cross-batch at the matched next step "
                        "t+1 + same-sequence other steps. 'all': the full "
                        "marginal proposal (van den Oord Eq. 4) — every e_{b',l} "
                        "over all sequences b' and all steps l, masking only the "
                        "positive (b,t+1). Cross-seq-all-time chunked via env "
                        "CPC_ALL_CHUNK (default 8).")
    ...
    for step in range(start_step + 1, args.total_steps + 1):
        ...
        with torch.amp.autocast('cuda', enabled=False):
            ...
            if args.cpc_infonce_weight > 0:
                cpc_fn = (cpc_infonce_all_loss if args.cpc_infonce_negs == "all"
                          else cpc_infonce_aux_loss)
                cpc_aux = cpc_fn(f_lat, o_lat, model.cpc_w1)
