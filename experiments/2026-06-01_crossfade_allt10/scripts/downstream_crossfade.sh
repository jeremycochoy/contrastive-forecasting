#!/bin/bash
# #325 — downstream for the crossfade backbone. IDENTICAL q-head recipe + GIFT-Eval to
# #322's downstream_b1024.sh (so the comparison to #322's allt·10% is clean) — only the
# backbone path / output dir are repointed. q-head batch 256; --num-layers 6 (6L
# forecaster) in both q-head training and eval; q-head depth (2|6) set by <head_layers>.
#
#   downstream_crossfade.sh <head_layers> <gpu> [evalsets]
set -uo pipefail
HL="${1:?head_layers}"; GPU="${2:?gpu}"; SETS="${3:-triage full}"
TAG="xshh_allt_forked10pct_crossfade10pct_qk_aon_b1024"
NAMEBB="bb_xshh_allt_forked10pct_crossfade10pct_qk_aon_6Lf_b1024"
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [xfade-dl ${HL}L g$GPU] $*"; }
