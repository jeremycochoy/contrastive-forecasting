...
a1.loglog(xs, [max(v, EPS) for v in xloss], color=XF_C, lw=1.7,
          label=f"allt·10% + crossfade·10% (#325) [{xf_done}]")
a1.loglog(bs, [max(v, EPS) for v in bloss], color=BASE_C, lw=1.5, ls="--",
          label="allt·10% (#322) [FINAL]")
...
a2.semilogx(xs, xgap, color=XF_C, lw=1.7, label="crossfade (#325)")
a2.semilogx(bs, bgap, color=BASE_C, lw=1.5, ls="--", label="allt·10% (#322)")
...
a3.semilogx(xs, xcb, color=XF_C, lw=1.7, label="crossfade (#325)")
a3.semilogx(bs, bcb, color=BASE_C, lw=1.5, ls="--", label="allt·10% (#322)")
...
fig.suptitle("#325 regime-crossfade backbone — training curves vs #322 allt·10%", fontsize=13)
