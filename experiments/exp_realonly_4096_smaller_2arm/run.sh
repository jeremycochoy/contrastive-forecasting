#!/bin/bash
# exp_realonly_4096_smaller_2arm — same as exp_realonly_4096_2arm but with a
# smaller backbone: L=6 H=384 ffn_mult=4 nhead=6 (vs Tiny's L=6 H=512 nhead=8).
#
# Purpose: under the realonly + T=4096 + C=1 setting, does a smaller
# architecture train better, equally, or worse than Tiny? Smaller has
# 11.4M params (vs Tiny's 19.96M, ~43% reduction), so per-step compute
# is lower and gradient noise dynamics differ.
#
# Compares vs exp_realonly_4096_2arm at the SAME 30k step budget on the
# SAME jeremycochoy/gift-pretrain-small-4096 dataset, with the SAME mix=0.0
# and --grad-clip 1.0. Only the backbone shape changes.
#
# Two arms (parallel on two machines):
#   - ewma128 arm: rev-norm-kind=ewma, span=128
#   - revin   arm: rev-norm-kind=revin
#
# Usage:
#   bash experiments/exp_realonly_4096_smaller_2arm/run.sh ewma128
#   bash experiments/exp_realonly_4096_smaller_2arm/run.sh revin
set -e
cd /workspace/app
...
LOG="/workspace/app/run_smaller_${ARM}.log"
exec >> >(tee -a "$LOG") 2>&1
echo "" && echo "=== run_realonly_4096_smaller: ARM ${ARM} starting ===" && date
...
# Smaller arch: L=6 H=384 nhead=6 (vs Tiny L=6 H=512 nhead=8)
BB="tiny_realonly_4096_smaller_${ARM}"
QH="R1q_realonly_4096_smaller_${ARM}"
RES_DIR="experiments/exp_realonly_4096_smaller_2arm/results/gift_eval_${ARM}"
mkdir -p "$RES_DIR"

# ===== Backbone =====
echo "" && echo "=== ARM ${ARM} STAGE B: $BB (L=6 H=384 nhead=6, T=4096, C=1, mix=0.0) ===" && date
...
echo "" && echo "=== run_realonly_4096_smaller: ARM ${ARM} ALL DONE ===" && date
echo ""
