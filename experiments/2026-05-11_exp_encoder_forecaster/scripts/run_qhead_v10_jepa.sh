#!/bin/bash
# v10 JEPA q-head — 2L causal transformer q-head on v10's JEPA backbone
# (encoder=6L, forecaster=1L). The tiny forecaster pushes semantic work
# into the encoder, so a thin q-head should suffice (JEPA convention,
# matches v9e's 2L head on v7 backbone for A/B).
#
# Recipe otherwise identical to v9e (R9_E13 + e_then_f + Moirai HP + cosine
# + 2k warmup, bf16, 30k steps). Backbone-arch flags adapted: --num-layers
# matches v10's forecaster depth (1), --encoder-type gru as in training.
# freq_emb_dim, seasonality_emb_dim, num_encoder_layers auto-detected from
# state dict (see train_forecasting_head.py lines 341-385).
#
# Compare triage GM-MASE to v7 (12L qh) = 1.512 and v9e (2L qh on v7) = 1.613.
#
# Run name: enc_fcst_v10jepa_qhead_xfmr2L_quant_30k.
set -e
...
echo "=== START $RUN_NAME (2L qhead on v10 JEPA backbone) ===" && date
