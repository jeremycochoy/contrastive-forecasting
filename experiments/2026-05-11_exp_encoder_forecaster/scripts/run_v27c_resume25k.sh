#!/bin/bash
# v11c — same as v11/v11b but fp16 body (resid+attn+ffn fp16, pemb fp32) — v27 RESUME from clean _25k.pth (post disk-full recovery) → 50k — dropkey sweep point + ffn-fp16 stability across the whole transformer body
...
NAME="enc_fcst_v27c_dk08_ffnfp16_resume25k_50k"
...
echo "=== START ${NAME} (fresh, JEPA 6L+1L, new conv, fp16 body (resid+attn+ffn fp16, pemb fp32) — v27 RESUME from clean _25k.pth (post disk-full recovery) → 50k — dropkey sweep point + ffn-fp16 stability) ===" && date
python3 -u experiments/2026-04-27_freq-embedding/scripts/train.py \
    ...
    --save-every 5000 --save-dir "${SAVE_DIR}" --run-name "${NAME}" --resume /home/jupyter/contrastive-forecasting/checkpoints/enc_fcst_v27b_dk08_ffnfp16_resume20k_50k_25k.pth \
