...
RUN_NAME="enc_fcst_v11c_qhead_xfmr2L_quant_30k_v2"
QHEAD_FINAL="$MAIN/checkpoints/${RUN_NAME}_FINAL.pth"
QHEAD_LOG="$EXP_DIR/results/run_${RUN_NAME}.log"
TRIAGE_OUT="$EXP_DIR/results/gift_eval_triage_v11c_v2"
TRIAGE_LOG="$EXP_DIR/results/run_triage_v11c_v2.log"
CHAIN_LOG="$EXP_DIR/results/post_qhead_chain_v11c_v2.log"

mkdir -p "$EXP_DIR/results"
echo "=== [v11c-v2 chain] START $(date) ===" | tee -a "$CHAIN_LOG"
...
if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v11c-v2 qhead] launching $RUN_NAME on GPU 0" | tee -a "$CHAIN_LOG"
    CUDA_VISIBLE_DEVICES=0 bash "$EXP_DIR/scripts/run_qhead_v11c_v2.sh" \
        >>"$CHAIN_LOG" 2>&1
    QHEAD_RC=$?
    if [ "$QHEAD_RC" != 0 ]; then
        echo "[v11c-v2 qhead] FAILED rc=$QHEAD_RC — chain aborting" | tee -a "$CHAIN_LOG"
        exit "$QHEAD_RC"
    fi
fi

if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v11c-v2 qhead] FINAL.pth still missing after training — abort" | tee -a "$CHAIN_LOG"
    exit 2
fi
echo "[v11c-v2 qhead] DONE — proceeding to triage" | tee -a "$CHAIN_LOG"
...
echo "[v11c-v2 triage] starting on GPU 0" | tee -a "$CHAIN_LOG"
PYTHONPATH="$ROOT" \
...
echo "[v11c-v2 triage] DONE — GM-MASE = ${GM:-?}  ($PASS vs v10=1.4369, v7=1.512)" | tee -a "$CHAIN_LOG"
echo "=== [v11c-v2 chain] DONE $(date) ===" | tee -a "$CHAIN_LOG"
