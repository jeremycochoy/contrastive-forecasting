#!/bin/bash
# v11c — same as v11/v11b but fp16 body (resid+attn+ffn fp16, pemb fp32) — fresh-init: attn fp32, rest fp16 — INSTRUMENTED: record per-layer attn-logit/activation amplitude through the fresh-init divergence across the whole transformer body
...
NAME="enc_fcst_v23_v22recipe_instrumented_50k"
...
echo "=== START ${NAME} (fresh, JEPA 6L+1L, new conv, fp16 body (resid+attn+ffn fp16, pemb fp32) — fresh-init: attn fp32, rest fp16 — INSTRUMENTED: record per-layer attn-logit/activation amplitude through the fresh-init divergence) ===" && date
python3 -u experiments/2026-04-27_freq-embedding/scripts/train.py \
    ...
    --encoder-type gru --log-attn-amplitude --log-attn-amplitude-every 200 \
