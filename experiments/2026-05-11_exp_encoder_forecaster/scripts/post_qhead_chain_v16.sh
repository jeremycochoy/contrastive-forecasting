#!/bin/bash
# v16 post-q-head chain: train 2L q-head on v16 JEPA backbone → triage.
...
TRIAGE_OUT="$EXP_DIR/results/gift_eval_triage_v16_jepa"
TRIAGE_LOG="$EXP_DIR/results/run_triage_v16.log"
CHAIN_LOG="$EXP_DIR/results/post_qhead_chain_v16.log"

mkdir -p "$EXP_DIR/results"
echo "=== [v16 chain] START $(date) ===" | tee -a "$CHAIN_LOG"
...
if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v16 qhead] launching $RUN_NAME on GPU 0" | tee -a "$CHAIN_LOG"
    CUDA_VISIBLE_DEVICES=0 bash "$EXP_DIR/scripts/run_qhead_v16.sh" \
        >>"$CHAIN_LOG" 2>&1
    QHEAD_RC=$?
    if [ "$QHEAD_RC" != 0 ]; then
        echo "[v16 qhead] FAILED rc=$QHEAD_RC — chain aborting" | tee -a "$CHAIN_LOG"
        exit "$QHEAD_RC"
    fi
fi

if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v16 qhead] FINAL.pth still missing after training — abort" | tee -a "$CHAIN_LOG"
    exit 2
fi
echo "[v16 qhead] DONE — proceeding to triage" | tee -a "$CHAIN_LOG"
...
echo "[v16 triage] starting on GPU 0" | tee -a "$CHAIN_LOG"
PYTHONPATH="$ROOT" \
...
echo "[v16 triage] DONE — GM-MASE = ${GM:-?}  ($PASS vs v11c=1.3878, v14=1.650, v15=1.671)" | tee -a "$CHAIN_LOG"
echo "=== [v16 chain] DONE $(date) ===" | tee -a "$CHAIN_LOG"
