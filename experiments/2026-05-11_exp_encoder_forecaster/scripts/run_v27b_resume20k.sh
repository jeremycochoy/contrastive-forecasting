#!/bin/bash
# v11c — same as v11/v11b but fp16 body (resid+attn+ffn fp16, pemb fp32) — v27 RESUME from clean _20k.pth (infra-killed at 24k, was stable) → 50k — dropkey sweep point + ffn-fp16 stability across the whole transformer body
...
NAME="enc_fcst_v27b_dk08_ffnfp16_resume20k_50k"
...
echo "=== START ${NAME} (fresh, JEPA 6L+1L, new conv, fp16 body (resid+attn+ffn fp16, pemb fp32) — v27 RESUME from clean _20k.pth (infra-killed at 24k, was stable) → 50k — dropkey sweep point + ffn-fp16 stability) ===" && date
python3 -u experiments/2026-04-27_freq-embedding/scripts/train.py \
    ...
    --save-every 5000 --save-dir "${SAVE_DIR}" --run-name "${NAME}" --resume /home/jupyter/contrastive-forecasting/checkpoints/enc_fcst_v27_dk08_ffnfp16_50k_20k.pth \
    --hf-repo "jeremycochoy/gift-pretrain-full-4096" --hf-path "small_v1" \
    --t-raw 4096 --n-channels 1 \
    --d-model 384 --n-heads 6 --num-layers 1 \
    --num-encoder-layers 6 --encoder-dropkey 0.8 \
    ...
    --encoder-type gru --log-attn-amplitude --log-attn-amplitude-every 1000 \
