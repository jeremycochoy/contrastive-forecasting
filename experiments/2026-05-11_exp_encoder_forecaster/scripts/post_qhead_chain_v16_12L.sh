#!/bin/bash
# v16 post-q-head chain: train 12L q-head on v16 JEPA backbone → triage.
...
RUN_NAME="enc_fcst_v16_qhead_xfmr12L_quant_30k"
QHEAD_FINAL="$MAIN/checkpoints/${RUN_NAME}_FINAL.pth"
QHEAD_LOG="$EXP_DIR/results/run_${RUN_NAME}.log"
TRIAGE_OUT="$EXP_DIR/results/gift_eval_triage_v16_jepa_12L"
TRIAGE_LOG="$EXP_DIR/results/run_triage_v16_12L.log"
CHAIN_LOG="$EXP_DIR/results/post_qhead_chain_v16_12L.log"

mkdir -p "$EXP_DIR/results"
echo "=== [v16-12L chain] START $(date) ===" | tee -a "$CHAIN_LOG"
...
if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v16-12L qhead] launching $RUN_NAME on GPU 0" | tee -a "$CHAIN_LOG"
    CUDA_VISIBLE_DEVICES=0 bash "$EXP_DIR/scripts/run_qhead_v16_12L.sh" \
        >>"$CHAIN_LOG" 2>&1
    QHEAD_RC=$?
    if [ "$QHEAD_RC" != 0 ]; then
        echo "[v16-12L qhead] FAILED rc=$QHEAD_RC — chain aborting" | tee -a "$CHAIN_LOG"
        exit "$QHEAD_RC"
    fi
fi

if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v16-12L qhead] FINAL.pth still missing after training — abort" | tee -a "$CHAIN_LOG"
    exit 2
fi
echo "[v16-12L qhead] DONE — proceeding to triage" | tee -a "$CHAIN_LOG"
...
echo "[v16-12L triage] starting on GPU 0" | tee -a "$CHAIN_LOG"
PYTHONPATH="$ROOT" \
...
echo "[v16-12L triage] DONE — GM-MASE = ${GM:-?}  ($PASS vs v11c=1.3878, v14=1.650, v15=1.671)" | tee -a "$CHAIN_LOG"
echo "=== [v16-12L chain] DONE $(date) ===" | tee -a "$CHAIN_LOG"
