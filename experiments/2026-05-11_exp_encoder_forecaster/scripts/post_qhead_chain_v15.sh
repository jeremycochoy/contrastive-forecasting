#!/bin/bash
# v15 post-q-head chain: train 2L q-head on v15 JEPA backbone (6L enc + 4L fcst)
# → triage on 11 GIFT-Eval short configs.
#
# Adapted from post_qhead_chain_v11c.sh — only paths/names updated and
# GPU pinned to 1 (v14 backbone is on GPU 0, do not touch).
set -uo pipefail
...
TRIAGE_OUT="$EXP_DIR/results/gift_eval_triage_v15_jepa"
TRIAGE_LOG="$EXP_DIR/results/run_triage_v15.log"
CHAIN_LOG="$EXP_DIR/results/post_qhead_chain_v15.log"

mkdir -p "$EXP_DIR/results"
echo "=== [v15 chain] START $(date) ===" | tee -a "$CHAIN_LOG"

# Step 1: q-head training. Pin GPU 1 (v14 lives on GPU 0).
if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v15 qhead] launching $RUN_NAME on GPU 1" | tee -a "$CHAIN_LOG"
    CUDA_VISIBLE_DEVICES=1 bash "$EXP_DIR/scripts/run_qhead_v15.sh" \
        >>"$CHAIN_LOG" 2>&1
    QHEAD_RC=$?
    if [ "$QHEAD_RC" != 0 ]; then
        echo "[v15 qhead] FAILED rc=$QHEAD_RC — chain aborting" | tee -a "$CHAIN_LOG"
        exit "$QHEAD_RC"
    fi
fi

if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v15 qhead] FINAL.pth still missing after training — abort" | tee -a "$CHAIN_LOG"
    exit 2
fi
echo "[v15 qhead] DONE — proceeding to triage" | tee -a "$CHAIN_LOG"
...
echo "[v15 triage] starting on GPU 1" | tee -a "$CHAIN_LOG"
PYTHONPATH="$ROOT" \
...
echo "[v15 triage] DONE — GM-MASE = ${GM:-?}  ($PASS vs v11c=1.388, v10=1.4369)" | tee -a "$CHAIN_LOG"
echo "=== [v15 chain] DONE $(date) ===" | tee -a "$CHAIN_LOG"
