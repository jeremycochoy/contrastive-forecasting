#!/bin/bash
# v11c — same as v11/v11b but fp16 body (resid+attn+ffn fp16, pemb fp32) — FFN-only fp16 (all else fp32) — most conservative partial-fp16, fresh init across the whole transformer body
...
NAME="enc_fcst_v26_ffnonly_fp16_50k"
...
echo "=== START ${NAME} (fresh, JEPA 6L+1L, new conv, fp16 body (resid+attn+ffn fp16, pemb fp32) — FFN-only fp16 (all else fp32) — most conservative partial-fp16, fresh init) ===" && date
