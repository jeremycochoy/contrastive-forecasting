...
B=enc_fcst_dk092_fp32_50k; TAG=dk092fp32_50k
QN=${TAG}_qhead_xfmr2L_quant_30k; QF="$CK/${QN}_FINAL.pth"; FOUT="$RES/gift_eval_full_${TAG}"
log "=== dk092_fp32_50k START (GPU0) ==="
if [ ! -f "$CK/${B}_FINAL.pth" ]; then
  log "$B from-scratch pure-fp32 dk0.92 seed$SEED 0->50k"
  python3 -u experiments/2026-04-27_freq-embedding/scripts/train.py "${COMMON[@]}" \
    --seed $SEED --encoder-dropkey 0.92 --total-steps 50000 --run-name "$B" \
...
log "=== dk092_fp32_50k DONE GM=$(gm "$FOUT/summary.txt") (vs 0.90/v11c=1.2920) ==="