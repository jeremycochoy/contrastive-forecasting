...
NEW_ARM_CSV = MAIN / "checkpoints/enc_fcst_dk09_hs_b512_50k_losses.csv"
...
ARMS = [
    ...
    ("attempt-1 (shared (T,T) dropkey mask, NaN'd @ 11700)",
     ...
     ATTEMPT1_CSV,
     False),
    ("attempt-2 (per-(B,head) dropkey, diverged @ ~14900)",
     C_ATTEMPT2, "-", 1.4,
     ATTEMPT2_CSV,
     False),
    ("attempt-3 (heads-shared, resumed from attempt-2 best, diverged @ ~14400)",
     ...
     ATTEMPT3_CSV,
     False),
    ("v4 fresh (B=512, dropkey=0.9 heads-shared)",
     C_HEADLINE_V4, "-", 1.8,
     NEW_ARM_CSV,
     False),
]
...
def render_figure(traj, *, logx, logy_metrics, out_path, scale_tag, xmax,
    ...
    fig.suptitle(
        f"encoder+forecaster — v4 B=512 dropkey=0.9 heads-shared (red) vs "
        f"attempt-3 heads-shared resume (purple) vs attempt-2 per-(B,head) "
        f"(olive) vs attempt-1 shared (T,T) (orange) vs τ=0.10 baseline "
...
def main() -> None:
    ...
    # (attempt-1 / attempt-2 / attempt-3 / v4-headline). All five arms
    ...
    xmax = max(new_arm_max, attempt1_max, attempt2_max, attempt3_max)
