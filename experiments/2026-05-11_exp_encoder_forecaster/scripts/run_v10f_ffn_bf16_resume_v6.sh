#!/bin/bash
# v10f — fp32 everywhere except the FFN x4 expansion+projection (bf16).
# Resume v6's _best_loss.pth (pre-divergence state, dropkey=0.9 hsl recipe)
# and train forward. Must clear v6's historical wall at step 4500 like v10d
# (pure fp32) did. If it clears, FFN-only-bf16 is a stable subset of fp32.
# (Likely-small speedup; this is primarily a stability test.)
#
# Run name: enc_fcst_v10f_ffn_bf16_resume_v6_15k. GPU 0.
...
NAME="enc_fcst_v10f_ffn_bf16_resume_v6_15k"
...
echo "=== START ${NAME} (resume v6 best_loss → step 15000, fp32 + FFN-bf16) ===" && date
