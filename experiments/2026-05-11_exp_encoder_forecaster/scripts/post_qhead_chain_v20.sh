#!/bin/bash
# v11c post-q-head chain: train 2L q-head on v20 (fp32-warmup→fp16) backbone → triage.
...
RUN_NAME="enc_fcst_v20_qhead_xfmr2L_quant_30k"
QHEAD_FINAL="$MAIN/checkpoints/${RUN_NAME}_FINAL.pth"
QHEAD_LOG="$EXP_DIR/results/run_${RUN_NAME}.log"
TRIAGE_OUT="$EXP_DIR/results/gift_eval_triage_v20"
TRIAGE_LOG="$EXP_DIR/results/run_triage_v20.log"
CHAIN_LOG="$EXP_DIR/results/post_qhead_chain_v20.log"

mkdir -p "$EXP_DIR/results"
echo "=== [v20 chain] START $(date) ===" | tee -a "$CHAIN_LOG"
...
if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v20 qhead] launching $RUN_NAME on GPU 0" | tee -a "$CHAIN_LOG"
    CUDA_VISIBLE_DEVICES=0 bash "$EXP_DIR/scripts/run_qhead_v20.sh" \
        >>"$CHAIN_LOG" 2>&1
    QHEAD_RC=$?
    if [ "$QHEAD_RC" != 0 ]; then
        echo "[v20 qhead] FAILED rc=$QHEAD_RC — chain aborting" | tee -a "$CHAIN_LOG"
        exit "$QHEAD_RC"
    fi
fi

if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v20 qhead] FINAL.pth still missing after training — abort" | tee -a "$CHAIN_LOG"
    exit 2
fi
echo "[v20 qhead] DONE — proceeding to triage" | tee -a "$CHAIN_LOG"
...
echo "[v20 triage] starting on GPU 0" | tee -a "$CHAIN_LOG"
PYTHONPATH="$ROOT" \
...
echo "[v20 triage] DONE — GM-MASE = ${GM:-?}  ($PASS vs v10=1.4369, v7=1.512)" | tee -a "$CHAIN_LOG"
echo "=== [v20 chain] DONE $(date) ===" | tee -a "$CHAIN_LOG"
