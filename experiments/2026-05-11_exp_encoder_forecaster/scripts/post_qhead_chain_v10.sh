#!/bin/bash
# v10 post-q-head chain: train 2L q-head on v10 JEPA backbone → triage.
# Adapted from post_qhead_chain_v9c.sh (triage settings) and run_qhead_v9c.sh
# (q-head launch). Q-head is 2L (JEPA-style, matches v9e), backbone is
# v10's encoder-6L + forecaster-1L JEPA backbone.
set -uo pipefail
...
BB_PATH="$MAIN/checkpoints/enc_fcst_v10_jepa_enc6_fcst1_50k_FINAL.pth"
RUN_NAME="enc_fcst_v10jepa_qhead_xfmr2L_quant_30k"
QHEAD_FINAL="$MAIN/checkpoints/${RUN_NAME}_FINAL.pth"
QHEAD_LOG="$EXP_DIR/results/run_${RUN_NAME}.log"
TRIAGE_OUT="$EXP_DIR/results/gift_eval_triage_v10jepa"
TRIAGE_LOG="$EXP_DIR/results/run_triage_v10jepa.log"
CHAIN_LOG="$EXP_DIR/results/post_qhead_chain_v10.log"

mkdir -p "$EXP_DIR/results"
echo "=== [v10 chain] START $(date) ===" | tee -a "$CHAIN_LOG"

# Step 1: kick off the q-head training (foreground inside this script).
# CUDA_VISIBLE_DEVICES=1 is set inside run_qhead_v10_jepa.sh's defaults.
if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v10 qhead] launching $RUN_NAME on GPU 1" | tee -a "$CHAIN_LOG"
    CUDA_VISIBLE_DEVICES=1 bash "$EXP_DIR/scripts/run_qhead_v10_jepa.sh" \
        >>"$CHAIN_LOG" 2>&1
    QHEAD_RC=$?
    if [ "$QHEAD_RC" != 0 ]; then
        echo "[v10 qhead] FAILED rc=$QHEAD_RC — chain aborting" | tee -a "$CHAIN_LOG"
        exit "$QHEAD_RC"
    fi
fi

if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v10 qhead] FINAL.pth still missing after training — abort" | tee -a "$CHAIN_LOG"
    exit 2
fi
echo "[v10 qhead] DONE — proceeding to triage" | tee -a "$CHAIN_LOG"

# Step 2: triage on 11 GIFT-Eval short configs. Same recipe as v9c chain
...
echo "[v10 triage] starting on GPU 1" | tee -a "$CHAIN_LOG"
PYTHONPATH="$ROOT" \
...
PASS=$(awk -v m="${GM:-99}" 'BEGIN{print (m+0 < 1.512) ? "BEAT_V7" : "WORSE_THAN_V7"}')
echo "[v10 triage] DONE — GM-MASE = ${GM:-?}  ($PASS vs v7=1.512, v9e=1.613)" | tee -a "$CHAIN_LOG"
echo "=== [v10 chain] DONE $(date) ===" | tee -a "$CHAIN_LOG"
