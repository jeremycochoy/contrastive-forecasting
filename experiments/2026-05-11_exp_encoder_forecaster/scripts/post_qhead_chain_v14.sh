#!/bin/bash
# v14 post-q-head chain: train 2L q-head on v14 JEPA backbone → triage.
# Adapted from post_qhead_chain_v11c.sh — same recipe, paths/names updated
# for v14, GPU pinned to 0 (GPU 1 may be running v15, do not touch).
# v14 differs from v11c by forecaster --num-layers 6 (vs 1L).
set -uo pipefail
...
TRIAGE_OUT="$EXP_DIR/results/gift_eval_triage_v14_jepa"
TRIAGE_LOG="$EXP_DIR/results/run_triage_v14.log"
CHAIN_LOG="$EXP_DIR/results/post_qhead_chain_v14.log"

mkdir -p "$EXP_DIR/results"
echo "=== [v14 chain] START $(date) ===" | tee -a "$CHAIN_LOG"

# Step 1: kick off the q-head training (foreground inside this script).
# CUDA_VISIBLE_DEVICES=0 — GPU 1 is occupied by v15, do not touch.
if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v14 qhead] launching $RUN_NAME on GPU 0" | tee -a "$CHAIN_LOG"
    CUDA_VISIBLE_DEVICES=0 bash "$EXP_DIR/scripts/run_qhead_v14.sh" \
        >>"$CHAIN_LOG" 2>&1
    QHEAD_RC=$?
    if [ "$QHEAD_RC" != 0 ]; then
        echo "[v14 qhead] FAILED rc=$QHEAD_RC — chain aborting" | tee -a "$CHAIN_LOG"
        exit "$QHEAD_RC"
    fi
fi

if [ ! -f "$QHEAD_FINAL" ]; then
    echo "[v14 qhead] FINAL.pth still missing after training — abort" | tee -a "$CHAIN_LOG"
    exit 2
fi
echo "[v14 qhead] DONE — proceeding to triage" | tee -a "$CHAIN_LOG"

# Step 2: triage on 11 GIFT-Eval short configs. Same recipe as v11c chain
...
echo "[v14 triage] starting on GPU 0" | tee -a "$CHAIN_LOG"
PYTHONPATH="$ROOT" \
...
echo "[v14 triage] DONE — GM-MASE = ${GM:-?}  ($PASS vs v11c=1.3878, v10=1.4369, v7=1.512)" | tee -a "$CHAIN_LOG"
echo "=== [v14 chain] DONE $(date) ===" | tee -a "$CHAIN_LOG"
