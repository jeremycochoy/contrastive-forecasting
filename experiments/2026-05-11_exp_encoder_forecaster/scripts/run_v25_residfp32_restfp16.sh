#!/bin/bash
# v11c — same as v11/v11b but fp16 body (resid+attn+ffn fp16, pemb fp32) — residual fp32, ALL else fp16 (attn+ffn+pemb) — predicted-safe max-speedup recipe, fresh init across the whole transformer body
...
NAME="enc_fcst_v25_residfp32_restfp16_50k"
...
echo "=== START ${NAME} (fresh, JEPA 6L+1L, new conv, fp16 body (resid+attn+ffn fp16, pemb fp32) — residual fp32, ALL else fp16 (attn+ffn+pemb) — predicted-safe max-speedup recipe, fresh init) ===" && date
python3 -u experiments/2026-04-27_freq-embedding/scripts/train.py \
    ...
    --patch-emb-dtype fp16 --residual-dtype fp32 --attn-dtype fp16 --ffn-dtype fp16 \
    --mix-ratio 0.0 \
