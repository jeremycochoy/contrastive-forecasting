#!/bin/bash
# Triage on v9a (60k q-head) + v7 backbone. Compare GM-MASE vs v7's 30k
# qhead = 1.512.
set -e
...
HEAD_NAME="enc_fcst_v7_qhead_xfmr12L_quant_60k"
BB_PATH="$MAIN/checkpoints/enc_fcst_dk09_hsl_b256_fp32_50k_FINAL.pth"
HEAD_PATH="$MAIN/checkpoints/${HEAD_NAME}_FINAL.pth"
OUT_DIR="$ROOT/experiments/2026-05-11_exp_encoder_forecaster/results/gift_eval_triage_v9a"
mkdir -p "$OUT_DIR"

[ -f "$BB_PATH" ] || { echo "ERROR: backbone missing at $BB_PATH" >&2; exit 1; }
[ -f "$HEAD_PATH" ] || { echo "ERROR: head missing at $HEAD_PATH" >&2; exit 1; }
...
echo "[v9a triage] using GPU 0 for $HEAD_NAME"
PYTHONPATH=. \
...
echo "[v9a triage] DONE  out=$OUT_DIR"
