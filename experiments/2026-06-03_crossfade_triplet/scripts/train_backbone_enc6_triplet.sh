#!/bin/bash
# #328 disentanglement — the missing cell: L6 + no-bottleneck + triplet
# (6-layer encoder + full-width forecaster + crossfade triplet). Same stabilised
# recipe / batch 1024 / 12500 steps / seed as every other arm; the ONLY changes
# vs base are: no bottleneck, 6-layer encoder kept, and --crossfade-triplets 1.
# Tests whether the triplet's full-training gain (seen on the 3L arm = #328) also
# holds with a full 6-layer encoder. Largest config + triplet, so it uses the same
# memory levers as the nobn arm (forecaster ckpt + all-time Gram chunk 1; byte-id).
#   train_backbone_enc6_triplet.sh <gpu> [steps] [save_every]
set -uo pipefail
...
NAME="bb_allt08_xftrip_nobn_enc6_qk_aon_b1024"
...
export FCST_GRAD_CKPT=1 XSHH_ALLT_CHUNK="${XSHH_ALLT_CHUNK:-1}"   # largest config: fit one 24 GB card (byte-identical)
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [enc6trip g$GPU] $*"; }
...
log "BB START enc=6 nobn triplet=1 bs=1024 steps=$STEPS ${RESUME}"
