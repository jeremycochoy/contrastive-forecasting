#!/bin/bash
# #328 — downstream for the triplet backbone. IDENTICAL q-head recipe + GIFT-Eval
# to #322 / #326 (so the comparison is clean) — only the backbone path / output
# dir are repointed, and the forecaster-bottleneck flags are DROPPED (this arm has
# none). The backbone's num_encoder_layers (3) and full-width forecaster are
# auto-detected from the checkpoint by train_forecasting_head.py / eval.
# q-head batch 256; backbone reconstructed with --num-layers 6 (forecaster depth).
#
#   downstream_triplet.sh <head_layers> <gpu> [evalsets]
set -uo pipefail
HL="${1:?head_layers}"; GPU="${2:?gpu}"; SETS="${3:-triage full}"
TAG="allt08_xftrip_nobn_enc3_qk_aon_b1024"
NAMEBB="bb_allt08_xftrip_nobn_enc3_qk_aon_b1024"
WT="${WT:-/tmp/cf-328}"
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [trip-dl ${HL}L g$GPU] $*"; }
gm(){ grep -E 'Aggregate GM-Relative MASE' "$1" 2>/dev/null | head -1 | grep -oE '[0-9]+\.[0-9]+' | head -1; }
[ -f "$BB" ] || { log "ABORT backbone missing: $BB"; exit 1; }
# Full-width forecaster + 3-layer encoder are auto-detected from the checkpoint;
# pass only the shared backbone geometry (no --forecaster-d-model/-n-heads).
