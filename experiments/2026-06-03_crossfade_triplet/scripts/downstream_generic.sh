#!/bin/bash
# #328 — generic downstream for a FULL-WIDTH backbone (no forecaster bottleneck):
# trains the best-loss q-head + GIFT-Eval full-97, then the full-training (last)
# q-head (resumed from best, re-adapted) + eval. Encoder depth / full width are
# auto-detected from the checkpoint, so this works for any full-width arm.
#   downstream_generic.sh <tag> <head_layers> <gpu> [fcst_dmodel]
#     backbone best = bb_<tag>_FINAL.pth ; last = bb_<tag>_final.pth
#     fcst_dmodel: pass 128 for a bottleneck backbone (adds --forecaster-d-model 128
#     --forecaster-n-heads 4 to q-head + eval); omit for full-width (auto-detected).
set -uo pipefail
TAG="${1:?tag}"; HL="${2:?head_layers}"; GPU="${3:?gpu}"; FCST_DM="${4:-}"
FCST=(); [ -n "$FCST_DM" ] && FCST=(--forecaster-d-model "$FCST_DM" --forecaster-n-heads 4)
WT="${WT:-/tmp/cf-328}"
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [dl-gen ${TAG##*_} ${HL}L g$GPU] $*"; }
...
else log "no final.pth ($BBLAST) — skipping last eval"; fi
log "generic downstream complete (${HL}L)"
