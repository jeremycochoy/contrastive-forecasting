...
# --- the replicate head-seed boxes ---------------------------------------
# These boxes run no ladder.py, so they have no ladder.csv to pull. What they
# hold is the evidence for the replicate seeds: the score file, the
# 97-config table, the summary and the encoder marker, under
# <cell>/eval/bb100k_<enc>_s<seed>/. Land it in elisa's durable root and the
# evidence copy above picks it up on the next pass, so the replicate rows are
# audited by exactly the same code as the seed-20260722 ones.
SEEDBOXES="$RES/seed_boxes.txt"
if [ -f "$SEEDBOXES" ] && [ -d "$ELISA_RUNS" ]; then
  while read -r lbl contract host port gpu cores rate cell seeds; do
    case "$lbl" in ''|\#*) continue ;; esac
    say "$lbl ($host:$port, $cell)"
    if ssh -n "${SSH_OPTS[@]}" -p "$port" "root@$host" \
         "cd $REMOTE_RUNS 2>/dev/null && tar czf /tmp/cf393_seed_evidence.tgz \
          \$(find $cell -path '*_s20260*' \( -name 'score_bb*.txt' \
            -o -name 'summary.txt' -o -name '*_encoder_source.txt' \
            -o -name 'all_results.csv' \) -type f) \
          \$(find $cell -maxdepth 2 -name 'score_bb*_s20260*.txt' -type f) \
          2>/dev/null" 2>/dev/null; then
      scp -q "${SSH_OPTS[@]}" -P "$port" \
          "root@$host:/tmp/cf393_seed_evidence.tgz" \
          "/tmp/cf393_seed_evidence_${lbl}.tgz" \
        && tar xzf "/tmp/cf393_seed_evidence_${lbl}.tgz" -C "$ELISA_RUNS" \
        && say "  seed evidence unpacked into the durable root"
    fi
    scp -q "${SSH_OPTS[@]}" -P "$port" \
        "root@$host:$REMOTE_EXP/results/seed_*.log" "$RES/seedboxes/" 2>/dev/null
  done < "$SEEDBOXES"
  # Re-run the elisa evidence copy so what just landed is in results/eval too.
  ( cd "$ELISA_RUNS" && find . \( -name 'score_bb*.txt' -o -name 'summary.txt' \
      -o -name '*_encoder_source.txt' -o -name 'all_results.csv' \) -type f \
      -exec cp -a --parents {} "$RES/eval/" \; ) 2>/dev/null
fi
...
# --- the head-seed spread, and which branches survive it -----------------
# The extend rule is a strict `<` on a difference whose noise it cannot see.
# This is the measurement of that noise inside this study, so it has to be
# rebuilt whenever a replicate seed lands, not once at the end.
...
# — with no committed audit at all. Run every cycle now, over whatever
# ladder_all.csv currently holds, so the audit cannot fall behind the table.
{ echo "# Regenerated by collect_results.sh over every row of ladder_all.csv."
  echo "# Supersedes audit_scores_bb40k.txt, which covered the 20 bb40k rows."
  echo "# $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
  echo
  python3 "$EXP/scripts/audit_scores.py" --results "$RES"
} > "$RES/audit_scores_all.txt" 2>&1
tail -1 "$RES/audit_scores_all.txt" | sed 's/^/  audit: /'

# --- the same, for the replicate head seeds ------------------------------
# More than the header: an empty replicate table would otherwise print
# "every published row backed: 0 summary check(s)", which reads like a pass.
if [ "$(wc -l < "$RES/seed_spread_rows.csv" 2>/dev/null || echo 0)" -gt 1 ]; then
  { echo "# Every replicate head-seed row traced to its GIFT-Eval summary."
    echo "# $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
    echo
    python3 "$EXP/scripts/audit_scores.py" --results "$RES" \
            --ladder "$RES/seed_spread_rows.csv"
  } > "$RES/audit_scores_seeds.txt" 2>&1
