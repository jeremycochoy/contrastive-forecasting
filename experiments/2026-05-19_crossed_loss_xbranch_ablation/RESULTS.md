...
*Radar chart, one spoke per GIFT-Eval domain; distance from centre = GM relative MASE, lower = better; dotted green = seasonal naive (1.0). Thick lines = #307 of-record arms; thin = #303 kept arms; **gold dashed** = best-ever (R9_E13, #127, full GM 1.029); **purple dashed** = v11c-recipe median, with the only band shown = its min/max across n=3 (1.292 / 1.323 / 1.333). Per-arm seed spread is **not** banded here (it lives in the candle plot above, which reads arm-vs-arm uncertainty far more clearly); the radar shows each arm's of-record curve so the per-domain shape stays legible. Econ/Fin is the dominant variance driver and the spoke where the arms separate most. **(B)+(C)** and **(A)+(B)+(C)** sit at or above the (A+B) cluster; (A+B+C) clusters with (A+B) on Econ/Fin (≈3.4) where A's harm is largest.*
