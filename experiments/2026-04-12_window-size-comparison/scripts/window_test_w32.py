#!/usr/bin/env python3
"""Window size experiment: W=32 baseline on Tiny backbone."""
...
W, H, L, C = 32, 512, 6, 4
T_RAW, BS, LR = 4096, 32, 1e-4
TOTAL_STEPS, VAL_EVERY = 30000, 1000
...
for step in range(1, TOTAL_STEPS + 1):
    ...
    if step % VAL_EVERY == 0 or step == TOTAL_STEPS:
        ...
        print(f"[{step:>6d}] loss={loss.item():.4f} FF={val_ff:.4f} FP={val_fp:.4f} gap={gap:.4f} best={best_gap:.4f} {sps:.1f}sps", flush=True)
print(f"\nW=32 DONE in {(time.time()-t0)/60:.1f}min. Best gap: {best_gap:.4f}", flush=True)
