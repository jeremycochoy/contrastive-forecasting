#!/usr/bin/env python3
"""#348 — GM-Relative MASE + paired-bootstrap Δ for the two no-encoder arms
(base, +CPC) vs their encoder'd #344 counterparts (enc3, enc6).

GM / paired-bootstrap logic is byte-for-byte #341/#344's analyze (so the numbers
are directly comparable to the stop-grad-capacity and CPC-aux reports, and the
baseline GMs reproduce to three decimals).

Depth ladder: num_encoder_layers ∈ {0 (noenc), 3 (enc3), 6 (enc6)} for each of
{base contrastive, + CPC}. Pairs answer #348:
  - does removing the encoder change transfer (base arm, vs enc3 and enc6)?
  - does removing the encoder change transfer (CPC arm, vs enc3 and enc6)?
  - does the CPC term still help WITHOUT the encoder (noenc base -> noenc cpc)?
...
ARMS = {
    ...
    "cpc_enc6":   ("enc6, + CPC (#344)",                 E344, "allt08_xftrip_nobn_enc6_sgpos_qk_aon_b1024_cpc"),
}

# (A, B, why) — Δ = GM(B) − GM(A); negative Δ ⇒ B better.
PAIRS = [
    # does removing the encoder improve transfer? (B = no-encoder)
    ("base_enc3", "noenc_base", "remove encoder, base arm (vs enc3) [hypothesis]"),
    ("base_enc6", "noenc_base", "remove encoder, base arm (vs enc6) [hypothesis]"),
    ("cpc_enc3",  "noenc_cpc",  "remove encoder, CPC arm (vs enc3) [hypothesis]"),
...
os.makedirs(E348, exist_ok=True)
for name, rows in (("gm_table.csv", gm_rows), ("pairwise_table.csv", pair_rows)):
    with open(f"{E348}/{name}", "w", newline="") as fh:
...
print()
print(f"{'A -> B':<26}{'head':<5}{'ckpt':<6}{'GM_A':>7}{'GM_B':>7}{'Δ':>9}{'  90% CI':>19}   verdict")
for r in pair_rows:
    ci = (f"({r['ci_lo']:+.3f},{r['ci_hi']:+.3f})" if r["ci_lo"] is not None else "(--)")
    print(f"{r['A']+' -> '+r['B']:<26}{r['head']:<5}{r['ckpt']:<6}"
          f"{fmt(r['gm_A']):>7}{fmt(r['gm_B']):>7}{fmt(r['delta'], '{:+.4f}'):>9}{ci:>19}   {r['verdict']}")
print(f"\nwrote {E348}/gm_table.csv and {E348}/pairwise_table.csv")
