#!/bin/bash
# Encoder + Forecaster (causal) — τ=0.10, 50k steps, on elisa GPU 1
#
# Architecture: GRU patch embedding → 6× causal transformer (encoder)
#               → 6× causal transformer (forecaster). The encoder's output
#               is the contrastive target (x_original is captured AFTER the
...
NAME="enc_fcst_tau_0_10_50k"
SAVE_DIR="/home/jupyter/contrastive-forecasting/checkpoints"
LOG_DIR="/home/jupyter/contrastive-forecasting/experiments/2026-05-10_exp_encoder_forecaster/results"
...
python3 -u experiments/2026-04-27_freq-embedding/scripts/train.py \
    ...
    --num-encoder-layers 6 --amp-dtype bf16 \
    --mix-ratio 0.0 \
