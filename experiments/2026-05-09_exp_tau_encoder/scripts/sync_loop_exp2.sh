#!/bin/bash
# Sync loop for Exp 2 (residual_silu encoder, τ=0.10) on a vast.ai 5090.
#
# Adapted from sync_loop_arm5_resync.sh. Differences:
# - Run name: exp2_residual_silu_tau_0_10 (not tau_sweep_0_20)
# - LOCAL_BASE: sync_exp2_residual_silu/
# - REMOTE_LOG: /workspace/app/run_exp2.log
# - Completion marker: "EXP2 DONE"
# - All atomic_scp / append-regression / per-class-size logic preserved.
#
# Usage: ./sync_loop_exp2.sh <SSH_HOST> <SSH_PORT> <INSTANCE_ID>
...
REMOTE_LOG="/workspace/app/run_exp2.log"
LOCAL_BASE="/home/jupyter/contrastive-forecasting/sync_exp2_residual_silu"
LOCAL_CKPT="${LOCAL_BASE}/checkpoints"
LOG_FILE="${LOCAL_BASE}/sync.log"
RUN_NAME="exp2_residual_silu_tau_0_10"

SSH_OPTS="-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=30 -o ServerAliveInterval=30"

mkdir -p "${LOCAL_CKPT}"

echo "[$(date)] Sync loop start exp2_residual_silu instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
...
while true; do
    ...
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === exp2_residual_silu cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    ALIVE=$(ssh ${SSH_OPTS} -p "${SSH_PORT}" "${REMOTE}" \
        'pgrep -af "python3.*train.py|run_exp2" >/dev/null && echo ALIVE || echo DEAD' \
        2>/dev/null | tail -1)
    echo "  remote training process: ${ALIVE:-UNREACHABLE}" | tee -a "${LOG_FILE}"

    # Completion marker for the exp2 launcher (run_exp2_residual_silu_vast.sh).
    if grep -q "EXP2 DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  === exp2 DONE detected; final sync ===" | tee -a "${LOG_FILE}"
        ...
        atomic_scp "${REMOTE_CKPT_DIR}/${BB}_losses.csv" \
                   "${LOCAL_CKPT}/${BB}_losses.csv" "${CSV_MIN}" || true
        atomic_scp "${REMOTE_LOG}" "${LOCAL_BASE}/run.log" "${LOG_MIN}" || true
        echo "[$(date)] Sync loop exp2_residual_silu finished" | tee -a "${LOG_FILE}"
        exit 0
    fi
