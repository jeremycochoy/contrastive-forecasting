#!/bin/bash
# #379 arm 1 — L_pred + L_rep, small backbone × 200k steps, on elisa.
#
# Same loss recipe as #374 arm 1 (split_pred_rep, no MoCo, no alignment
# extras). Only the backbone architecture (d_model=128, n_heads=16,
...
DL_LOG="$RES/dl_arm1.log"
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [elisa-arm1] $*" | tee -a "$DL_LOG"; }
...
if [ -f "$BB" ]; then
  ...
  log "BB START (arm 1: split_pred_rep) gpu=$BB_GPU steps=$STEPS bs=128 ${RESUME}"
...
downstream_hl 2 "$GPU_2L" >>"$RES/dl_2L_arm1.log" 2>&1 & pid2=$!
downstream_hl 6 "$GPU_6L" >>"$RES/dl_6L_arm1.log" 2>&1 & pid6=$!
