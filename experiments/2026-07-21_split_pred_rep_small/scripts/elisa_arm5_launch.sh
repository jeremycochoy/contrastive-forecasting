#!/bin/bash
# #379 arm 5 — L_align + L_rep, small backbone × 200k steps, on elisa.
#
# Same loss recipe as #374 arm 5 (rep_only shape + --align-loss-weight 1.0;
# BYOL-style alignment on top of the h-anchored repulsion term). Only the backbone architecture (d_model=128, n_heads=16,
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [elisa-arm5] $*" | tee -a "$DL_LOG"; }
...
if [ -f "$BB" ]; then
  ...
  log "BB START (arm 5: rep_only + align-loss-weight 1.0) gpu=$BB_GPU steps=$STEPS bs=128 ${RESUME}"
...
downstream_hl 2 "$GPU_2L" >>"$RES/dl_2L_arm5.log" 2>&1 & pid2=$!
downstream_hl 6 "$GPU_6L" >>"$RES/dl_6L_arm5.log" 2>&1 & pid6=$!
