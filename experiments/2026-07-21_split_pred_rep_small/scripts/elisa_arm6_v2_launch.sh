#!/bin/bash
# #379 arm 6 v2 — L_align + L_rep_moco, small backbone × 200k steps, on elisa.
#
# Same loss recipe as #374 arm 6 v2 (rep_only shape + --align-loss-weight 1.0
# + --moco-rep-keys; BYOL alignment + MoCo-keyed repulsion on L_rep, no
# MoCo on L_pred and no L_pred at all — rep_only drops the f-anchored term). Only the backbone architecture (d_model=128, n_heads=16,
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [elisa-arm6_v2] $*" | tee -a "$DL_LOG"; }
...
if [ -f "$BB" ]; then
  ...
  log "BB START (arm 6 v2: rep_only + align-loss-weight 1.0 + moco-rep-keys) gpu=$BB_GPU steps=$STEPS bs=128 ${RESUME}"
...
downstream_hl 2 "$GPU_2L" >>"$RES/dl_2L_arm6_v2.log" 2>&1 & pid2=$!
downstream_hl 6 "$GPU_6L" >>"$RES/dl_6L_arm6_v2.log" 2>&1 & pid6=$!
