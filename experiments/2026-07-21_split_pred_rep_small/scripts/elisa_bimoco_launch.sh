#!/bin/bash
# #379 bimoco — L_pred_moco + L_rep_moco (split + MoCo on both terms),
# small backbone × 200k steps, on elisa.
#
# Same loss recipe as #374 bimoco (split_pred_rep + --moco-negatives
# + --moco-rep-keys — MoCo teacher-keyed contrastive on both f- and
# h-anchored families, with the fixed same-batch same-time
# student↔teacher positive on L_rep). Only the backbone architecture (d_model=128, n_heads=16,
...
if [ -f "$BB" ]; then
  ...
  log "BB START (bimoco: split_pred_rep + moco-negatives + moco-rep-keys) gpu=$BB_GPU steps=$STEPS bs=128 ${RESUME}"
...
downstream_hl 2 "$GPU_2L" >>"$RES/dl_2L_bimoco.log" 2>&1 & pid2=$!
downstream_hl 6 "$GPU_6L" >>"$RES/dl_6L_bimoco.log" 2>&1 & pid6=$!
