...
**What**: same A/B as `exp_csb_pair_span512` but with **RevIN** as the
...
**Why**: combined with `exp_csb_pair_span512` this gives a **2 × 2 grid**
...
**Setup**: see `run.sh`. Same protocol as `exp_csb_pair_span512` (30k bb
...
shared `_aggregate/results/synth_eval.csv`.
...
- 2 new rows in `../_aggregate/results/synth_eval.csv`.
...
instance that ran `exp_csb_pair_span512` (deps already installed, raw
