...
![Generated samples and their true correlation matrices](figures/data_samples_v5.png)
...
![Training curves: backbone gap and recovery head loss](figures/training_curves_v5.png)
...
![True vs predicted, sample-level](figures/true_vs_predicted_bars_v5.png)

![Per-pair scatter](figures/scatter_per_pair_v5.png)

The scatter now covers the full `[0, 1]` range with broadly even density — the
uniform sampler does what it says.

![Error distributions](figures/error_distributions_v5.png)

![Predicted vs ground-truth correlation matrices](figures/corr_matrix_heatmaps_v5.png)

![Head vs analytic baselines](figures/baseline_comparison_v5.png)
...
- `experiments/contrastive-correlation/joint_channel_model.py` —
  `JointChannelModel` and `GRUCorrelationHead`.
- `experiments/contrastive-correlation/train_joint_channel.py` — backbone
  training (constant LR by default).
- `experiments/contrastive-correlation/train_joint_channel_head.py` — head
  training; reuses the V4 baselines (diff(x), x corrcoef) for evaluation.
- `experiments/contrastive-correlation/evaluate_joint_channel.py` — figure
  generation.
...
- `experiments/contrastive-correlation/figures/*_v5.png` — figures used here.
