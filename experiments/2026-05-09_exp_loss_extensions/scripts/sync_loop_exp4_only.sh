#!/bin/bash
# Single-arm sync loop for the Exp 4 STANDALONE (non-cumulative) vast.ai run.
# Adapted verbatim from
# experiments/2026-05-09_exp_loss_extensions/scripts/sync_loop_exp4.sh.
#
# Differences:
# - LOCAL_BASE = sync_exp4_only_fnegs/ (separate from the cumulative variant
#   sync_exp4_fnegs/ and any τ-sweep targets)
# - Run name   = exp4_only_fnegs_tau_0_20
# - REMOTE_LOG = /workspace/app/run_exp4_only_fnegs.log (matches the launcher's
#   redirect target).
# - Completion marker grep is "EXP4-ONLY DONE".
...
#
# Usage: ./sync_loop_exp4_only.sh <SSH_HOST> <SSH_PORT> <INSTANCE_ID>
...
REMOTE_LOG="/workspace/app/run_exp4_only_fnegs.log"
LOCAL_BASE="/home/jupyter/contrastive-forecasting/sync_exp4_only_fnegs"
...
echo "[$(date)] Sync loop start exp4_only_fnegs instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
    ...
    if scp ${SSH_OPTS} -P "${SSH_PORT}" -q "${REMOTE}:${remote_path}" "${tmp}" 2>/dev/null; then
        if [ -s "${tmp}" ]; then
            ...
            if [ "${sz}" -ge "${min_size}" ]; then
                    ...
                    *.csv|*.log)
                        if [ -s "${local_dest}" ]; then
                            ...
                            if [ "${_new_lines}" -lt "${_old_lines}" ]; then
                                ...
                                mv "${tmp}" "${_archived}"
                                echo "  [warn] APPEND REGRESSION on ${_bn}: remote=${_new_lines} lines, local=${_old_lines} lines — archived to ${_archived}, NOT rotating." | tee -a "${LOG_FILE}"
                                return 0
                            fi
                        fi
                        ;;
                ...
                echo "  fail TOO SMALL $(basename "${remote_path}") (${sz} < ${min_size})" | tee -a "${LOG_FILE}"
                rm -f "${tmp}"
                return 1
            fi
        else
...
ARMS=(0_20)
RUN_PREFIX="exp4_only_fnegs_tau"

cycle_count=0
while true; do
    cycle_count=$((cycle_count + 1))
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === exp4_only_fnegs cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    if grep -qiE "nan detected|Traceback|Error:" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  [warn] NaN/Error/Traceback in run.log" | tee -a "${LOG_FILE}"
    fi

    ALIVE=$(ssh ${SSH_OPTS} -p "${SSH_PORT}" "${REMOTE}" \
        'pgrep -af "python3.*train.py|run_exp4_only" >/dev/null && echo ALIVE || echo DEAD' \
        2>/dev/null | tail -1)
    echo "  remote training process: ${ALIVE:-UNREACHABLE}" | tee -a "${LOG_FILE}"

    # Completion marker for the exp4 standalone launcher.
    if grep -q "EXP4-ONLY DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  === exp4_only_fnegs DONE detected; final sync ===" | tee -a "${LOG_FILE}"
        for ARM in "${ARMS[@]}"; do
            BB="${RUN_PREFIX}_${ARM}"
        ...
        atomic_scp "${REMOTE_LOG}" "${LOCAL_BASE}/run.log" "${LOG_MIN}" || true
        echo "[$(date)] Sync loop exp4_only_fnegs finished" | tee -a "${LOG_FILE}"
        exit 0
    fi
