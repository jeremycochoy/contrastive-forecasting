#!/bin/bash
# Single-arm sync loop for the Exp 4 cumulative-loss vast.ai run.
# Adapted from
# experiments/2026-05-08_exp_tau_sweep/scripts/sync_loop_arm5_resync.sh.
#
# Differences:
# - LOCAL_BASE = sync_exp4_fnegs/ (separate from any τ-sweep targets)
# - Run name   = exp4_fnegs_tau_0_20
# - REMOTE_LOG = /workspace/app/run_exp4_fnegs.log (matches the launcher's
#   redirect target).
...
#
# Usage: ./sync_loop_exp4.sh <SSH_HOST> <SSH_PORT> <INSTANCE_ID>
...
REMOTE_LOG="/workspace/app/run_exp4_fnegs.log"
LOCAL_BASE="/home/jupyter/contrastive-forecasting/sync_exp4_fnegs"
...
echo "[$(date)] Sync loop start exp4_fnegs instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
...
ARMS=(0_20)
RUN_PREFIX="exp4_fnegs_tau"

cycle_count=0
while true; do
    cycle_count=$((cycle_count + 1))
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === exp4_fnegs cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    ALIVE=$(ssh ${SSH_OPTS} -p "${SSH_PORT}" "${REMOTE}" \
        'pgrep -af "python3.*train.py|run_exp4" >/dev/null && echo ALIVE || echo DEAD' \
        2>/dev/null | tail -1)
    echo "  remote training process: ${ALIVE:-UNREACHABLE}" | tee -a "${LOG_FILE}"

    # Completion marker for the exp4 launcher.
    if grep -q "EXP4 DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  === exp4_fnegs DONE detected; final sync ===" | tee -a "${LOG_FILE}"
        for ARM in "${ARMS[@]}"; do
            BB="${RUN_PREFIX}_${ARM}"
        ...
        atomic_scp "${REMOTE_LOG}" "${LOCAL_BASE}/run.log" "${LOG_MIN}" || true
        echo "[$(date)] Sync loop exp4_fnegs finished" | tee -a "${LOG_FILE}"
        exit 0
    fi
