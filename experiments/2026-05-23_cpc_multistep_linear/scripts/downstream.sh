#!/bin/bash
# #316 downstream: train a quantile forecasting q-head on a (frozen) CPC
# backbone, then run GIFT-Eval triage(11) + full(97). The backbone's
# linear_cpc forecaster is auto-detected from the checkpoint
# (transformer.cpc_heads.* keys) by BOTH the head-trainer and the eval, so no
# forecaster flags are passed here. extract_forecaster_latents returns the
# k=1 head — the same single forecaster latent the β downstream used — so the
# protocol matches #309/#315 and the GM-MASE numbers line up with β / v11c.
#
#   downstream.sh <backbone_path> <head_layers> <gpu> [filter]
#     backbone_path = .../bb_cpc_..._FINAL.pth (or a periodic _Nk.pth)
#     head_layers   = 2 (small head, #309 default) | 6 (6L head, #313)
#     gpu           = 0 | 1   (CUDA_VISIBLE_DEVICES)
#     filter        = triage | full | both (default both)
set -uo pipefail
BB="${1:?backbone_path}"; HL="${2:?head_layers (2|6)}"; GPU="${3:?gpu_id}"; WHICH="${4:-both}"
...
[ -f "$BB" ] || { echo "ABORT: backbone $BB missing"; exit 1; }
BASE="$(basename "$BB" .pth)"            # e.g. bb_cpc_k12_s20260520_fp16_50k_FINAL
TAG="${BASE}_h${HL}L"
QN="${TAG}_qhead_xfmr${HL}L_quant_30k"
...
qhead(){
  ...
  log "QH START ${HL}L 30k on GPU$GPU"
...
log "=== START downstream $TAG (head=${HL}L) ==="
qhead || { log "ABORT qhead failed"; exit 1; }
[ "$WHICH" = "triage" -o "$WHICH" = "both" ] && do_eval "triage_${TAG}" "$RES/gift_eval_triage_${TAG}" "$TRIAGE" || true
[ "$WHICH" = "full"   -o "$WHICH" = "both" ] && do_eval "full_${TAG}"   "$RES/gift_eval_full_${TAG}"   ""        || true
log "=== COMPLETE $TAG triageGM=$(gm "$RES/gift_eval_triage_${TAG}/summary.txt") fullGM=$(gm "$RES/gift_eval_full_${TAG}/summary.txt") ==="
