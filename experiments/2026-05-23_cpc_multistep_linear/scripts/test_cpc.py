...
Run BEFORE any GPU launch to de-risk the multi-hour training run. Checks:
  1. linear_cpc model builds; transformer.forward(return_multi) shapes.
  ...
  5. state_dict has cpc_heads.* and NO transformer.layers.* / fcst_*proj;
     round-trips through a freshly-built linear_cpc model (load strict).
...
loss.backward()
g_head = m.transformer.cpc_heads[0].weight.grad
...
m2 = make_model("linear_cpc").train()
...
sd = m.state_dict()
has_heads = [k for k in sd if k.startswith("transformer.cpc_heads.")]
...
assert has_heads, "no cpc_heads in state_dict"
...
assert not any(k.startswith("transformer.cpc_heads.") for k in sdt)
