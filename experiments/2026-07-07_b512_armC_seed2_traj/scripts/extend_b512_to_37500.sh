#!/bin/bash
# #371 — resume the B=512 arm-C seed2 backbone from step 25000 and train
# to step 37500 (matches #369 B=1024 x37.5k so 2L/6L curves compare
# head-to-head at 30k / 35k / 37.5k). Same recipe as train_b512_seed2.sh
# — only --resume and --total-steps change. Trajectory saves preserved.
#
#   WT=/home/jupyter/cf-371 EXP=<this experiment dir> GPU=1 bash extend_b512_to_37500.sh
set -uo pipefail
: "${WT:?}"; : "${EXP:?}"
GPU="${GPU:-1}"
STEPS="${STEPS:-37500}"
...
RESUME="$SAVE_DIR/${NAME_BASE}_step25000.pth"
...
[ $((STEPS % TRAJ_SAVE_EVERY)) -eq 0 ] && [ "$STEPS" -gt 25000 ] \
  || { echo "ABORT: STEPS=$STEPS violates traj-save invariant (>25000, multiple of $TRAJ_SAVE_EVERY)" >&2; exit 2; }
...
tlog="$RES/run_bb_extend37500_${NAME_BASE}.log"
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [bb-b512-ext37500 g$GPU] $*"; }
