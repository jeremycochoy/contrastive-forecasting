#!/bin/bash
# Single-arm composite-v2B-seasheavy driver. Identical to v2pulse run.sh
# except passes --seas-heavy instead of --enable-pulse.
#
# Targets the wave-dilution losses identified in phase-1: swaps
# (2 free + 1 seas-tied) waves to (1 free + 2 seas-tied), so periodic
# signal dominates the channel when seas-tied is on.
#
# Usage:
#   bash experiments/exp_compositesynth_v2bseasheavy_2arm/run.sh revin
#   bash experiments/exp_compositesynth_v2bseasheavy_2arm/run.sh ewma128
set -e
cd /workspace/app
...
exec >> >(tee -a "$LOG") 2>&1
echo "" && echo "=== run_compositesynth_v2bseasheavy: ARM ${ARM} starting ===" && date

SETUP_MARKER="/workspace/app/.setup_done_compositesynth_v2"
...
BB="tiny_compsyn_v2b_${ARM}"
QH="R1q_compsyn_v2b_${ARM}"
RES_DIR="experiments/exp_compositesynth_v2bseasheavy_2arm/results/gift_eval_${ARM}"
mkdir -p "$RES_DIR"
...
python3 -u experiments/freq-embedding/scripts/train.py \
    ...
    --mix-ratio 0.5 --synth-kind composite --seas-heavy \
...
echo "" && echo "=== run_compositesynth_v2bseasheavy: ARM ${ARM} ALL DONE ===" && date
echo ""
