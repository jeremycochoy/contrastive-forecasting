#!/bin/bash
# Single-arm resync variant of sync_loop.sh — for the τ=0.20 (arm5) recovery run
# launched on a fresh vast.ai 5090 instance after the previous arm5 lost its
# trajectory CSV when its instance was preempted (no sync_loop in place).
#
# Why a separate script? The canonical sync_loop.sh is built for the 5-arm
# sequential sweep launched by run_tau_sweep.sh — it pulls the SHARED remote
# log /workspace/app/run_tau_sweep.log into LOCAL_BASE/run.log. Elisa is
# CURRENTLY training arm 4 (τ=0.10) and writing to
# /home/jupyter/contrastive-forecasting/sync_tau_sweep/run.log. Running the
# canonical sync_loop here would clobber elisa's live training log on every
# tick (mv -f run.log run.log.prev; mv tmp run.log).
#
# Differences from canonical sync_loop.sh:
# - LOCAL_BASE points at sync_tau_sweep_arm5_resync/ (separate from elisa's
#   sync_tau_sweep/)
# - Only ARM 0_20 is iterated
# - REMOTE_LOG is /workspace/app/run_tau_0_20.log (matches arm5 launcher's
#   redirect target — the launcher does NOT write run_tau_sweep.log)
# - All other behaviour (atomic_scp, append-regression guard, completion
#   marker detection, per-class size thresholds) is preserved verbatim.
#
# Usage: ./sync_loop_arm5_resync.sh <SSH_HOST> <SSH_PORT> <INSTANCE_ID>
...
LOCAL_BASE="/home/jupyter/contrastive-forecasting/sync_tau_sweep_arm5_resync"
...
echo "[$(date)] Sync loop start tau_sweep_arm5_resync instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
...
# Per-class size thresholds (same as canonical sync_loop).
BB_MIN=40000000
BB_OPT_MIN=70000000
CSV_MIN=1
LOG_MIN=1
...
while true; do
    ...
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === tau_sweep_arm5_resync cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    # Completion marker for the single-arm vast launcher (run_tau_sweep_vast_arm5.sh).
    if grep -q "ARM τ=0.20 DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  === arm5 (τ=0.20) DONE detected; final sync ===" | tee -a "${LOG_FILE}"
        for ARM in "${ARMS[@]}"; do
            BB="tau_sweep_${ARM}"
        ...
        atomic_scp "${REMOTE_LOG}" "${LOCAL_BASE}/run.log" "${LOG_MIN}" || true
        echo "[$(date)] Sync loop tau_sweep_arm5_resync finished" | tee -a "${LOG_FILE}"
        exit 0
    fi
