#!/bin/bash
# Single-arm variant of sync_loop_arm5_resync.sh — for the τ=0.20 v2 retrain
# launched on a fresh vast.ai 5090 spot 36367883 after the previous arm5 lost
# its trajectory CSV. Pulls run_name=tau_sweep_0_20_v2_* (note the _v2 suffix
# so we don't overwrite the existing tau_sweep_0_20_FINAL.pth in
# sync_tau_sweep/checkpoints/).
#
# Differences from sync_loop_arm5_resync.sh:
# - LOCAL_BASE points at sync_tau_sweep_arm5_v2/
# - REMOTE_LOG is /workspace/app/run_tau_0_20_v2.log
# - Filenames pulled are tau_sweep_0_20_v2_*
#
# Usage: ./sync_loop_arm5_v2.sh <SSH_HOST> <SSH_PORT> <INSTANCE_ID>
...
REMOTE_LOG="/workspace/app/run_tau_0_20_v2.log"
LOCAL_BASE="/home/jupyter/contrastive-forecasting/sync_tau_sweep_arm5_v2"
...
echo "[$(date)] Sync loop start tau_sweep_arm5_v2 instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
...
# Single arm; run_name = tau_sweep_0_20_v2 on the vast launcher.
BB="tau_sweep_0_20_v2"

cycle_count=0
while true; do
    cycle_count=$((cycle_count + 1))
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === tau_sweep_arm5_v2 cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    if grep -q "ARM τ=0.20 DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  === arm5 v2 (τ=0.20) DONE detected; final sync ===" | tee -a "${LOG_FILE}"
        ...
        atomic_scp "${REMOTE_CKPT_DIR}/${BB}_losses.csv" \
                   "${LOCAL_CKPT}/${BB}_losses.csv" "${CSV_MIN}" || true
        atomic_scp "${REMOTE_LOG}" "${LOCAL_BASE}/run.log" "${LOG_MIN}" || true
        echo "[$(date)] Sync loop tau_sweep_arm5_v2 finished" | tee -a "${LOG_FILE}"
        exit 0
    fi
