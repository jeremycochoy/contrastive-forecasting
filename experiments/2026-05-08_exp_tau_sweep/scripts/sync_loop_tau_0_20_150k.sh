#!/bin/bash
# Single-arm sync loop for the τ=0.20 50k → 150k continuation run on a
# fresh vast.ai DC GPU on-demand instance. Pulls run_name=tau_sweep_0_20_150k_*
# files into /home/jupyter/contrastive-forecasting/sync_tau_sweep_0_20_150k/.
#
# Identical to the τ=0.10 sister sync loop, only paths/names differ.
#
# Usage: ./sync_loop_tau_0_20_150k.sh <SSH_HOST> <SSH_PORT> <INSTANCE_ID>
...
REMOTE_LOG="/workspace/app/run_tau_0_20_150k.log"
LOCAL_BASE="/home/jupyter/contrastive-forecasting/sync_tau_sweep_0_20_150k"
...
echo "[$(date)] Sync loop start tau_sweep_0_20_150k instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
...
BB="tau_sweep_0_20_150k"

cycle_count=0
while true; do
    cycle_count=$((cycle_count + 1))
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === tau_sweep_0_20_150k cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    ALIVE=$(ssh ${SSH_OPTS} -p "${SSH_PORT}" "${REMOTE}" \
        'pgrep -af "python3.*train.py|run_tau_0_20_resume_150k" >/dev/null && echo ALIVE || echo DEAD' \
        2>/dev/null | tail -1)
    echo "  remote training process: ${ALIVE:-UNREACHABLE}" | tee -a "${LOG_FILE}"

    if grep -q "ARM τ=0.20 150k DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  === τ=0.20 150k arm DONE detected; final sync ===" | tee -a "${LOG_FILE}"
        ...
        atomic_scp "${REMOTE_CKPT_DIR}/${BB}_losses.csv" \
                   "${LOCAL_CKPT}/${BB}_losses.csv" "${CSV_MIN}" || true
        atomic_scp "${REMOTE_LOG}" "${LOCAL_BASE}/run.log" "${LOG_MIN}" || true
        echo "[$(date)] Sync loop tau_sweep_0_20_150k finished" | tee -a "${LOG_FILE}"
        exit 0
    fi
