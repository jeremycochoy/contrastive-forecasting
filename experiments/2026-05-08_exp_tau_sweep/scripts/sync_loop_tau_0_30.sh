#!/bin/bash
# Single-arm sync loop for the τ-sweep extension run (τ=0.30, 15k steps) on
# a fresh vast.ai 5090 DC on-demand instance. Pulls run_name=tau_sweep_0_30_*
# files into /home/jupyter/contrastive-forecasting/sync_tau_sweep_0_30/.
#
# Atomic per-class size-gated scp with append-regression guard for CSV/log
# (CLAUDE.md sync rules).
#
# Usage: ./sync_loop_tau_0_30.sh <SSH_HOST> <SSH_PORT> <INSTANCE_ID>
...
REMOTE_LOG="/workspace/app/run_tau_0_30.log"
LOCAL_BASE="/home/jupyter/contrastive-forecasting/sync_tau_sweep_0_30"
...
echo "[$(date)] Sync loop start tau_sweep_0_30 instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
...
# Per-class size thresholds (same as canonical sync_loop). C=1, d_model=384,
# 6L backbone is ~46 MB (.pth) and ~90 MB (_optimizer.pth) at this arch.
BB_MIN=40000000
BB_OPT_MIN=70000000
CSV_MIN=1
LOG_MIN=1

# Single arm; run_name = tau_sweep_0_30 on the vast launcher.
BB="tau_sweep_0_30"
...
while true; do
    ...
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === tau_sweep_0_30 cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    ALIVE=$(ssh ${SSH_OPTS} -p "${SSH_PORT}" "${REMOTE}" \
        'pgrep -af "python3.*train.py|run_tau_0_30" >/dev/null && echo ALIVE || echo DEAD' \
        2>/dev/null | tail -1)
    echo "  remote training process: ${ALIVE:-UNREACHABLE}" | tee -a "${LOG_FILE}"

    if grep -q "ARM τ=0.30 DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  === τ=0.30 arm DONE detected; final sync ===" | tee -a "${LOG_FILE}"
        ...
        atomic_scp "${REMOTE_CKPT_DIR}/${BB}_losses.csv" \
                   "${LOCAL_CKPT}/${BB}_losses.csv" "${CSV_MIN}" || true
        atomic_scp "${REMOTE_LOG}" "${LOCAL_BASE}/run.log" "${LOG_MIN}" || true
        echo "[$(date)] Sync loop tau_sweep_0_30 finished" | tee -a "${LOG_FILE}"
        exit 0
    fi
