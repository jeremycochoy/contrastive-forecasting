#!/bin/bash
# τ sweep extension — vast.ai single-arm launcher (τ=0.80 only).
#
# Adds a τ=0.80 point to the existing τ-sweep. Recipe is identical to
# run_tau_0_30_vast.sh except --tau is 0.80 and the run-name is
# tau_sweep_0_80. Trains 15k steps on a vast.ai 5090 data-center on-demand
# instance.
#
...
#
# Idempotent: exits immediately if checkpoints/tau_sweep_0_80_FINAL.pth
# already exists.
#
# Usage (on the vast container):
#   nohup bash experiments/2026-05-08_exp_tau_sweep/scripts/run_tau_0_80_vast.sh \
#       > /workspace/app/run_tau_0_80.log 2>&1 &
#   disown
...
echo "" && echo "=== ARM τ=0.80 DONE ===" && date
