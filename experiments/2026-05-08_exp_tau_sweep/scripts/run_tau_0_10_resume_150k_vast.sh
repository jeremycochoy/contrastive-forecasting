#!/bin/bash
# τ=0.10 resume — 50k → 150k vast.ai single-arm launcher.
#
# Continue τ=0.10 from its 50k FINAL bundle to 150,000 steps total on a
# fresh vast.ai DC GPU on-demand instance. Three more "doublings" past
# the 50k window so we can see whether the τ=0.10 trajectory keeps
# moving past plateau or stays flat through 100k+.
#
# Resume bundle (model + optimizer + step counter + RNG state) is pushed
# by the caller from elisa to /workspace/app/checkpoints/ before launch:
#   checkpoints/tau_sweep_0_10_resume_150k.pth           (= local _r2_FINAL.pth)
#   checkpoints/tau_sweep_0_10_resume_150k_optimizer.pth (= local _r2_FINAL_optimizer.pth)
# Distinct `_resume_150k` suffix keeps `safe_run_name()` from auto-branching
# the new run-name (`tau_sweep_0_10_150k`) — outputs land directly under
# the requested name, no `_r2` suffix.
#
# Robustness: 5-attempt retry loop (60 s sleep between) for HF / network blips.
# Idempotent: exits if checkpoints/tau_sweep_0_10_150k_FINAL.pth already exists.
#
# Usage (on the vast container):
#   nohup bash experiments/2026-05-08_exp_tau_sweep/scripts/run_tau_0_10_resume_150k_vast.sh \
#       > /workspace/app/run_tau_0_10_150k.log 2>&1 &
#   disown
...
TAU="0.10"
NAME="tau_sweep_0_10_150k"
RESUME="checkpoints/tau_sweep_0_10_resume_150k.pth"
MAX_RETRIES=5
RETRY_SLEEP=60
...
# If a periodic save from a prior failed attempt on this instance exists,
# resume from the largest <run>_<step>k.pth that has a companion _optimizer.pth.
# Otherwise fall back to the bundle pushed in pre-launch.
detect_resume() {
    local latest=""
    local latest_step=-1
...
while [ "$attempt" -le "$MAX_RETRIES" ]; do
    ...
    if [ -z "$RESUME_PATH" ]; then
        # Fall back to the best_loss from the in-progress run if it exists
        # (after a preempt/restart the resume bundle may still be in place
        # but a more-recent best_loss will have higher step).
...
# pulls both files with the expected names. If safe_run_name() did kick in
# (e.g. resume bundle was named with the run-name prefix on a re-launch),
# pick up the actual training-output namespace.
PROMOTE_NAME="${NAME}"
...
echo "" && echo "=== ARM τ=0.10 150k DONE ===" && date
