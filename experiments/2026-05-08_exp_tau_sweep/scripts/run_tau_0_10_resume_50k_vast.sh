#!/bin/bash
# τ=0.10 resume — vast.ai single-arm launcher.
#
# Continue τ=0.10 from its 15k FINAL.pth to 50,000 steps total on a fresh
# vast.ai DC GPU on-demand instance (RTX 5090 / 4090 / A6000 / L40), so we
...
#   checkpoints/tau_sweep_0_10_50k_best_loss.pth
#   checkpoints/tau_sweep_0_10_50k_best_loss_optimizer.pth
# (re-provision after May 9 2026 preemption — resume from local best_loss
#  at step ~24500 rather than the original 15k FINAL bundle.)
#
# Robustness: 5-attempt retry loop (60 s sleep between) for HF / network blips.
# Idempotent: exits if checkpoints/tau_sweep_0_10_50k_FINAL.pth already exists.
#
# Usage (on the vast container):
#   nohup bash experiments/2026-05-08_exp_tau_sweep/scripts/run_tau_0_10_resume_50k_vast.sh \
#       > /workspace/app/run_tau_0_10_50k.log 2>&1 &
#   disown
...
RESUME="checkpoints/tau_sweep_0_10_50k_best_loss.pth"
MAX_RETRIES=5
RETRY_SLEEP=60
...
if [ -f "${SAVE_DIR}/${NAME}_FINAL.pth" ]; then
    echo "=== SKIP τ=${TAU} 50k (${SAVE_DIR}/${NAME}_FINAL.pth exists) ===" && date
    exit 0
fi
...
echo "=== ARM τ=${TAU} RESUME → 50k → run_name=${NAME} ===" && date
...
while [ "$attempt" -le "$MAX_RETRIES" ]; do
    ...
    python3 -u experiments/2026-04-27_freq-embedding/scripts/train.py \
        --device cuda:0 --total-steps 50000 --batch-size 256 \
...
if [ "$rc" -ne 0 ]; then
    echo "=== ARM τ=${TAU} 50k FAILED after ${MAX_RETRIES} attempts (rc=${rc}) ===" && date
    exit "$rc"
fi
...
echo "" && echo "=== ARM τ=0.10 50k DONE ===" && date
