#!/bin/bash
# Single-arm sync loop for the τ=0.10 50k continuation run on a fresh
# vast.ai DC GPU on-demand instance. Pulls run_name=tau_sweep_0_10_50k_*
# files into /home/jupyter/contrastive-forecasting/sync_tau_sweep_0_10_50k/.
#
# Re-provision after May 9 2026 preemption — current vast assignment:
#   instance 36419425 / ssh6.vast.ai:19424 / DE / RTX 5090 / on-demand DC.
#
# Distinct LOCAL_BASE so we don't overwrite the existing 15k checkpoints
# in sync_tau_sweep/.
#
# Atomic per-class size-gated scp with append-regression guard for CSV/log
# (CLAUDE.md sync rules).
#
# Usage: ./sync_loop_tau_0_10_50k.sh <SSH_HOST> <SSH_PORT> <INSTANCE_ID>
...
REMOTE_LOG="/workspace/app/run_tau_0_10_50k.log"
LOCAL_BASE="/home/jupyter/contrastive-forecasting/sync_tau_sweep_0_10_50k"
...
echo "[$(date)] Sync loop start tau_sweep_0_10_50k instance=${INSTANCE_ID} @ ${SSH_HOST}:${SSH_PORT}" | tee -a "${LOG_FILE}"
...
# Per-class size thresholds (canonical sync_loop). C=1, d_model=384, 6L
# backbone is ~46 MB (.pth) and ~90 MB (_optimizer.pth) at this arch.
BB_MIN=40000000
BB_OPT_MIN=70000000
CSV_MIN=1
LOG_MIN=1

# Single arm; run_name = tau_sweep_0_10_50k on the vast launcher.
...
# tau_sweep_0_10_50k_r2_*. We track that namespace here.
BB="tau_sweep_0_10_50k_r2"

cycle_count=0
while true; do
    cycle_count=$((cycle_count + 1))
    echo "" | tee -a "${LOG_FILE}"
    echo "[$(date)] === tau_sweep_0_10_50k cycle #${cycle_count} ===" | tee -a "${LOG_FILE}"
    ...
    ALIVE=$(ssh ${SSH_OPTS} -p "${SSH_PORT}" "${REMOTE}" \
        'pgrep -af "python3.*train.py|run_tau_0_10_resume_50k" >/dev/null && echo ALIVE || echo DEAD' \
        2>/dev/null | tail -1)
    echo "  remote training process: ${ALIVE:-UNREACHABLE}" | tee -a "${LOG_FILE}"

    if grep -q "ARM τ=0.10 50k DONE" "${LOCAL_BASE}/run.log" 2>/dev/null; then
        echo "  === τ=0.10 50k arm DONE detected; final sync ===" | tee -a "${LOG_FILE}"
        ...
        atomic_scp "${REMOTE_CKPT_DIR}/${BB}_losses.csv" \
                   "${LOCAL_CKPT}/${BB}_losses.csv" "${CSV_MIN}" || true
        atomic_scp "${REMOTE_LOG}" "${LOCAL_BASE}/run.log" "${LOG_MIN}" || true
        echo "[$(date)] Sync loop tau_sweep_0_10_50k finished" | tee -a "${LOG_FILE}"
        exit 0
    fi
