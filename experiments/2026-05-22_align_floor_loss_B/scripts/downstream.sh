#!/bin/bash
# #313 downstream: q-head 30k 2L-causal quantile + GIFT-Eval triage(11)
# + full(97) for the (B)+align+floor backbone. Same protocol as #309
# downstream.sh so numbers are comparable to (B)/β/γ. bneck arch.
#
#   downstream.sh <gpu_id>     gpu_id = 0 | 1  (CUDA_VISIBLE_DEVICES)
set -uo pipefail
GPU="${1:?gpu_id}"
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [alignfloor g$GPU] $*"; }
...
log "=== START downstream alignfloor (bneck=${FCST[*]}) ==="
qhead || { log "ABORT qhead failed"; exit 1; }
do_eval "triage" "$RES/gift_eval_triage_${NAME}" "$TRIAGE" || true
do_eval "full"   "$RES/gift_eval_full_${NAME}"   ""        || true
log "=== COMPLETE alignfloor triageGM=$(gm "$RES/gift_eval_triage_${NAME}/summary.txt") fullGM=$(gm "$RES/gift_eval_full_${NAME}/summary.txt") ==="
