...
otherwise-frozen best arm from `exp_span_sweep_synth` (fe+mu mix=1.0
...
- The original span=512 baseline checkpoint was lost when an auxiliary
  ...
  recurrence).
- The original CSB run (`exp_csb_synth`) had three remote-instance failures
...
`../_aggregate/results/synth_eval.csv`. Numbers are directly comparable
