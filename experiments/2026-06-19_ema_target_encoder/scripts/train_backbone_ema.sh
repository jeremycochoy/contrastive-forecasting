#!/bin/bash
# #353 — EMA-target (BYOL/JEPA) on the enc3+CPC arm from #344.
# Single arm: full-width forecaster, enc3 encoder, GRU patch-embed; the
# EXACT #344 enc3+CPC recipe with two flag swaps:
#   --stopgrad-positive-h      DROPPED (the teacher is the implicit stop-grad)
#   --ema-embedding            ADDED
#   --ema-encoder              ADDED
#   --ema-tau 0.99             ADDED (constant, no schedule)
# The main-contrastive positive's encoder side h_{t+1} is sourced from the
# slow-moving teacher (EMA of patch-embed + 3L encoder). Negatives, forecaster,
# and CPC term stay on the student. Single GPU.
#
#   train_backbone_ema.sh <gpu> [steps] [save_every]
set -uo pipefail
...
# Same memory knobs as the #344 enc3+CPC arm.
ENC_LAYERS=6; NENC=3
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [bb-ema-enc3 g$GPU] $*"; }
...
log "BB START arm=ema-enc3 bs=1024 steps=$STEPS xshh_chunk=$XSHH_ALLT_CHUNK cpc_chunk=$CPC_CB_CHUNK ${RESUME}"
