#!/bin/bash
# #353 — downstream q-head + GIFT-Eval for the EMA-target enc3+CPC backbone.
# Identical protocol to #344's downstream_cpc.sh (the comparison must be
# head-matched against the #344 enc3+CPC baseline). Trains best-loss q-head
# (30k) then last-checkpoint q-head (resumed, 10k re-adapt), per head_layers,
# then runs full-97 GIFT-Eval for each cell.
#
#   downstream_ema.sh <head_layers: 2|6> <gpu>
set -uo pipefail
HL="${1:?head_layers}"; GPU="${2:?gpu}"
TAG="allt08_xftrip_nobn_enc3_emateach_qk_aon_b1024_cpc"
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [dl-ema-enc3 ${HL}L g$GPU] $*"; }
...
log "ema downstream complete (${HL}L)"
