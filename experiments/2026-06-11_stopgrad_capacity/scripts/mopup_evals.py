#!/usr/bin/env python3
"""Mop-up missing configs of a sharded GIFT-Eval cell (#341, cloned from #339).
...
names, which are partially uppercase), and runs N mop-up shard processes
restricted to exactly those raw names. usage: mopup_evals.py <head_run_name>
<bb_file> <out_tag> <head_layers> <n_shards> <gpu_list_csv>
"""
import csv
import glob
import os
...
WT = "/tmp/cf-341"
EVAL_SH = f"{WT}/experiments/2026-06-11_stopgrad_capacity/scripts/eval_one_sgcap.sh"
sys.path.insert(0, f"{WT}/experiments/2026-06-11_stopgrad_capacity/scripts")
