...
* `2026-04-28_exp_csb_pair_span512` (synth held-out): RevEWMNorm **span=512** wins.
* `2026-04-27_exp_span_sweep_real` (real-data contrastive ema_loss): RevEWMNorm **span=128** wins.
...
2. **EWMA span=512 wins on median and on the `<1.5` count.** It is the
   ...
   `2026-04-27_exp_span_sweep_real` sweep, but here both winners are EWMA, not RevIN.
...
`2026-04-27_exp_span_sweep_real` showed loss preferred span=128 but gap preferred
...
2. **Bundle freq plumb**: rewrite `base_mixed_v2` with per-row
   ...
   ~62 % → 100 %.
3. **EWMA span=64 / span=256**: the U-curve from `2026-04-27_exp_span_sweep_real`
