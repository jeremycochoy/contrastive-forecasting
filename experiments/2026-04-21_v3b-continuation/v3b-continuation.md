...
It did not. v3b reached **GM-Relative MASE 1.186** (97 GIFT-Eval configs) — indistinguishable from the v2-resumed v3 (1.188) and slightly *worse* than v2 itself (1.168). Training from scratch on real data did not help.
...
| v3 (v2-resumed on real bundles) | 500k | 1.188 |
...
v3b beats seasonal-naive only on the non-periodic tail (jena_weather/10T 0.59, us_births/D 0.66, sz_taxi ≈ 0.85). The red band is the failure mode: m4_hourly 4.4×, solar/10T 2.0–3.4×, electricity/15T 2.8× the seasonal-naive error. The model has not learned periodic structure from the real data alone.
...
Real-world data, on its own, does not teach this backbone periodicity: the periodic-dataset failure survived the switch from synthetic-then-resume to from-scratch-on-real. That is the result that motivated the next experiment, [periodic-synth-mix](../2026-04-27_periodic-synth-mix/periodic-synth-mix.md), which injects the structure the real bundles lack by mixing 50% on-the-fly periodic synthetic series (sinusoid / square / saw) into training.

## Caveats

- **Under target.** The backbone was shelved at ~120k of its 500k target after repeated Vast.ai preemptions, so the comparison to the 500k v2/v3 baselines is not apples-to-apples — v3b is under-trained, and the honest reading is "no improvement *so far*," not a settled ceiling. The operational detail (preemption chain, ~$14 spent, the seven vastrun-kit reliability bugs filed) is in [notes/EXECUTION_LOG.md](notes/EXECUTION_LOG.md).
