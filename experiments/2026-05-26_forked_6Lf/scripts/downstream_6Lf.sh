#!/bin/bash
# #320 — downstream for the 6L-FORECASTER forked backbones. Lifted verbatim from
# #318's downstream_6Lf.sh (the data-side counterpart), only OUT/WT repointed to
# this card's dir. Backbone arch uses --num-layers 6 (matches the 6L-forecaster
# training) in BOTH q-head training and eval; q-head depth (2|6) is independent
# and set by <head_layers>. --amp-dtype none avoids the depthwise-conv fp32/bf16
# mismatch in extract_forecaster_latents (the #318 bf16 crash).
#
#   downstream_6Lf.sh <backbone_path> <head_layers> <tag> <gpu> <evalsets>
set -uo pipefail
