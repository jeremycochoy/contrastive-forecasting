"""Comparison plot: composite-v2pulse (this exp) vs composite (phase-1)
vs periodic (phase-0 baseline), at matched norms (RevIN and EWMA-128).

Reads six CSVs:
  experiments/exp_compositesynth_v2pulse_2arm/results/gift_eval_{revin,ewma128}/all_results.csv  (this exp)
...
Run from the repo root:
    python experiments/exp_compositesynth_v2pulse_2arm/scripts/plot_compare_2arm.py
"""
...
OUT = HERE / "plots" / "gift_eval_v2pulse_compare.png"
...
def main() -> None:
    arms = [
        ("v2pulse_revin",  RESULTS_DIR / "gift_eval_revin" / "all_results.csv",
            "v2pulse + RevIN",     "#8B0000"),
        ("v2pulse_ewma128", RESULTS_DIR / "gift_eval_ewma128" / "all_results.csv",
            "v2pulse + EWMA-128",  "#FF4500"),
    ...
    ax = axes[1, 1]
    pairs = []
    if "v2pulse_ewma128" in data and "comp_ewma128" in data:
        pairs.append(("v2pulse vs composite EWMA-128",
                      "v2pulse_ewma128", "comp_ewma128", "#FF4500"))
    if "v2pulse_ewma128" in data and "base_ewma128" in data:
        pairs.append(("v2pulse vs periodic EWMA-128",
                      "v2pulse_ewma128", "base_ewma128", "#2ca02c"))
    if pairs:
        lo, hi = 0.3, 200
        ...
        for label, comp_key, base_key, c in pairs:
            ...
            ax.scatter(xs, ys, s=14, alpha=0.7, color=c,
                       label=f"{label}: v2pulse wins {wins}/{len(xs)}")
        ax.set_xscale("log")
        ax.set_yscale("log")
        ax.set_xlim(lo, hi)
        ax.set_ylim(lo, hi)
        ax.set_xlabel("Reference (composite or periodic) MASE")
        ax.set_ylabel("v2pulse-EWMA-128 MASE")
        ax.set_title("v2pulse-EWMA-128 head-to-head")
        ax.grid(alpha=0.3)
    ...
    for label, ck, bk, _ in pairs:
        ...
        print(f"  {label}: v2pulse-EWMA-128 beats reference on {(ys < xs).sum()}/{len(xs)}")
