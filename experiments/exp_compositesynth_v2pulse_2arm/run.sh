#!/bin/bash
# Single-arm composite-v2-pulse driver. Designed for parallel execution
...
#
# Usage:
#   bash experiments/exp_compositesynth_v2pulse_2arm/run.sh revin
#   bash experiments/exp_compositesynth_v2pulse_2arm/run.sh ewma128
set -e
cd /workspace/app
...
exec >> >(tee -a "$LOG") 2>&1
echo "" && echo "=== run_compositesynth_v2pulse: ARM ${ARM} starting ===" && date
...
QH="R1q_compsyn_v2pulse_${ARM}"
RES_DIR="experiments/exp_compositesynth_v2pulse_2arm/results/gift_eval_${ARM}"
mkdir -p "$RES_DIR"
...
python3 -u experiments/freq-embedding/scripts/train.py \
    ...
    --mix-ratio 0.5 --synth-kind composite --enable-pulse \
...
echo "" && echo "=== run_compositesynth_v2pulse: ARM ${ARM} ALL DONE ===" && date
echo ""
