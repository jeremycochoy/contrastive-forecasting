#!/bin/bash
# #363 — downstream q-head + GIFT-Eval for one SIGReg λ-sweep arm. Identical
# protocol to #359 / #355 downstream_sigreg.sh: trains best-loss q-head (30k)
# then last-checkpoint q-head (resumed, 10k re-adapt), per head_layers, then
...
log "sigreg downstream complete (${HL}L for ${SUFFIX})"
