#!/bin/bash
# #318 follow-up — 6-LAYER-FORECASTER variant of the two arms (user request).
# Identical to the 1L-forecaster recipe (β + the arm's cross-series edit)
# EXCEPT --num-layers 6 (forecaster stack 1→6; the 6L causal encoder
# --num-encoder-layers 6 is unchanged). Run all-time FIRST (most negatives).
#
# Usage: train_backbone_6Lf.sh <arm> [gpu] [chunk]
#   arm    samestep | alltime
#   gpu    CUDA_VISIBLE_DEVICES (default 0)
#   chunk  XSHH_ALLT_CHUNK for the all-time arm (default 8)
set -uo pipefail
ARM="${1:?arm = samestep|alltime}"; GPU="${2:-0}"; export XSHH_ALLT_CHUNK="${3:-8}"
case "$ARM" in
  samestep) SHAPE=cosine_similarity_batch_full_hh_negs_xshh;      NAME=bb_xshh_6Lf_50k ;;
  alltime)  SHAPE=cosine_similarity_batch_full_hh_negs_xshh_allt; NAME=bb_xshh_allt_6Lf_50k ;;
...
log(){ echo "[$(date '+%m-%d %H:%M:%S')] [6Lf-$ARM] $*"; }
[ -f "$BB" ] && { log "BB SKIP ($NAME FINAL exists)"; exit 0; }
tlog="$RES/run_${NAME}.log"; export CUDA_VISIBLE_DEVICES="$GPU"
log "BB START shape=$SHAPE num-layers=6 (forecaster) chunk=$XSHH_ALLT_CHUNK GPU=$GPU -> 50000"
python3 -u "$TRAIN" \
