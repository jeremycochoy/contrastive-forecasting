#!/bin/bash
# #318 follow-up — downstream for the 6L-FORECASTER backbones. Identical to
# downstream.sh except the backbone arch uses --num-layers 6 (matches the
# 6L-forecaster training) in BOTH q-head training and eval. q-head depth (2|6)
# is independent and still set by <head_layers>.
#
...
# NOTE: --num-layers 6 here (vs 1 in downstream.sh) — the 6L-forecaster arch.
