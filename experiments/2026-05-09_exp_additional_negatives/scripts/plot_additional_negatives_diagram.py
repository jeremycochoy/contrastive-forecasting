...
Each vertex = prediction pair (f_{b,τ-1} → h_{b,τ}).  Renders TWO PNGs:

- additional_negatives_diagram.png:        each candidate-new edge drawn once,
  at the centred anchor s = t-1 — easier to read.
- additional_negatives_diagram_echoes.png: each colour also drawn at every
  in-window translation copy (anchor s ± 1, plus b ↔ b' label-swap for
  cross-batch terms) — more complete view of what one loss term creates when
  summed over all anchors and (b, b') pairs.

Existing covered edges are shown muted for context.
"""
import matplotlib
matplotlib.use("Agg")
...
for v in ("TL", "TM", "TR", "BL", "BM", "BR"):
    NODES[f"f_{v}"] = (CX[v] - D, CY[v])
    NODES[f"h_{v}"] = (CX[v] + D, CY[v])
...
def draw_diagram(echoes: bool, output_path: str) -> None:
    """Render one diagram. ``echoes=True`` adds every translation copy of each
    new-edge colour that fits the 6-vertex window, plus the b↔b' mirror of
    the existing orange diag block."""
    fig, ax = plt.subplots(figsize=(14, 9))
    ax.set_xlim(-1.0, 16.0)
    ax.set_ylim(0.5, 12.5)
    ax.axis("off")

    def arrow(src, dst, color, lw, arrowhead=False, ls="-",
              rad=0.0, alpha=1.0, zorder=3):
    ...
    # ── EXISTING: neg_cross_batch_forecast_embedding (f→h same col, cross-b) ──
    # Easier-to-read view: only the top-f↔bottom-h direction (3 edges).
    # Echoes view: also draw the b↔b' mirror bottom-f↔top-h (3 more edges) —
    # the cross-batch sum runs over every ordered (b, b') and produces both.
    for col in ("L", "M", "R"):
    ...
    # ── NEW UNCOVERED (centred anchor s = t-1) ────────────────────────────────
    ...
    # ── AUTO-SHIFTED ECHOES (echoes view only) ────────────────────────────────
    # Every translation copy of each new-edge colour that lands in the window:
    # anchor s ± 1, plus b ↔ b' label-swap for cross-batch terms. Same colour
    # as the source edge; legend unchanged.
    if echoes:
        # WB1 (purple) — within-b (f_τ, h_τ); τ ∈ {t-1, t}, both rows
        arrow("f_TM", "h_TL", C_WB1, 2.1, ls=(0, (4, 3)), rad=-0.40, zorder=4)
        arrow("f_BM", "h_BL", C_WB1, 2.1, ls=(0, (4, 3)), rad= 0.40, zorder=4)
        arrow("f_BR", "h_BM", C_WB1, 2.1, ls=(0, (4, 3)), rad= 0.40, zorder=4)
        # WB2 (magenta) — within-b (f_τ, h_{τ-1}); only τ=t fits both rows
        arrow("f_BR", "h_BL", C_WB2, 2.1, ls=(0, (4, 3)), rad= 0.28, zorder=4)
        # CB0 (teal) — cross-b (h_b, f_{b'}) same τ; τ ∈ {t-1, t}
        arrow("h_BL", "f_TM", C_CB0, 2.1, ls=(0, (4, 3)), rad=-0.22, zorder=4)
        arrow("h_TM", "f_BR", C_CB0, 2.1, ls=(0, (4, 3)), rad= 0.22, zorder=4)
        arrow("h_BM", "f_TR", C_CB0, 2.1, ls=(0, (4, 3)), rad=-0.22, zorder=4)
        # CB1 (orange) — same loss term as CB0 under b↔b'
        arrow("f_TM", "h_BL", C_CB1, 2.1, ls=(0, (4, 3)), rad= 0.18, zorder=4)
        arrow("f_BM", "h_TL", C_CB1, 2.1, ls=(0, (4, 3)), rad=-0.18, zorder=4)
        arrow("f_BR", "h_TM", C_CB1, 2.1, ls=(0, (4, 3)), rad=-0.18, zorder=4)
        # CB2 (dark red) — cross-b (f_τ, h_{τ-1}); only τ=t fits
        arrow("f_BR", "h_TL", C_CB2, 2.1, ls=(0, (4, 3)), rad=-0.10, zorder=4)
        # CTH (olive) — cross-b (h_{τ-1}, h_τ); τ ∈ {t, t+1}
        arrow("h_BL", "h_TM", C_CTH, 2.1, ls=(0, (4, 3)), rad= 0.18, zorder=4)
        arrow("h_TM", "h_BR", C_CTH, 2.1, ls=(0, (4, 3)), rad=-0.18, zorder=4)
        arrow("h_BM", "h_TR", C_CTH, 2.1, ls=(0, (4, 3)), rad= 0.18, zorder=4)
        # CTF (brown) — cross-b (f_τ, f_{τ+1}); τ ∈ {t-2, t-1}
        arrow("f_TL", "f_BM", C_CTF, 2.1, ls=(0, (4, 3)), rad= 0.30, zorder=4)
        arrow("f_BL", "f_TM", C_CTF, 2.1, ls=(0, (4, 3)), rad=-0.30, zorder=4)
        arrow("f_BM", "f_TR", C_CTF, 2.1, ls=(0, (4, 3)), rad=-0.30, zorder=4)
    ...
    for k, (x, y) in NODES.items():
        ...
        ax.scatter(x, y, s=440, zorder=6, color=c,
    ...
    for k, (x, y) in NODES.items():
        ...
        ax.text(x, y + dy, NL[k], ha="center", va=va,
                fontsize=8.5, fontweight="bold")

    # ── Column / row headers ──────────────────────────────────────────────────
    for cx, lbl in ((CX_L, r"time $t\!-\!1$"),
                    (CX_M, r"time $t$"),
                    (CX_R, r"time $t\!+\!1$")):
        ax.text(cx, CY_T + 2.0, lbl, ha="center", fontsize=10.5,
    ...
    ax.text(1.2, CY_T, "batch $b$",  ha="left", va="center",
            fontsize=9, color="#555")
    ax.text(1.2, CY_B, "batch $b'$", ha="left", va="center",
            fontsize=9, color="#555")
...
if __name__ == "__main__":
    base = "experiments/2026-05-09_exp_additional_negatives/plots"
    draw_diagram(echoes=False, output_path=f"{base}/additional_negatives_diagram.png")
    draw_diagram(echoes=True,  output_path=f"{base}/additional_negatives_diagram_echoes.png")
