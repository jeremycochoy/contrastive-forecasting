...
capacity. Each layer first applies a small depthwise causal
convolution (kernel size $3$) directly to the stream, then a
self-attention block, then a feed-forward network. The attention and
feed-forward sub-layers follow the standard pre-norm pattern with
residual connections; the depthwise convolution is applied without a
residual wrap.
