...
The negative pool is built by varying anchors along three independent
axes: cross-temporal ($t \neq t'$), cross-channel ($c \neq c'$), and
cross-batch ($b \neq b'$). Each axis can be combined with the others,
and each pair can mix encoded latents $h$ and forecasts $f$. The
current loss keeps two specific pair families.
\begin{itemize}
\item \textbf{Cross-channel} ($\mathcal{N}_{\mathrm{xx}}$): two
...
Many other pairings are conceivable. The cross-temporal axis suggests
$\big(h[b, t, c],\, h[b, t+1, c]\big)$,
$\big(h[b, t, c],\, f[b, t, c]\big)$, or
...
with a channel difference. An earlier version of the loss carried the
cross-temporal terms; we removed them after observing that the
additional compute was not justified: the cross-temporal terms
produced a slightly lower training loss but no significant difference
at convergence.
...
multiplications that the GPU executes in parallel. At every position
$(t, c)$ we aggregate the candidates of all anchors into two sums,
...
\]
and the two negative sums at position $(t, c)$ are
\begin{align*}
