...
encoder. In theory the encoder could be as complex as a transformer,
if encoding a patch into a useful latent turns out to be a hard
problem in its own right. Finding the proper encoder is a potential
future-work axis.
...
forecast $f[b, t, c]$ paired with the future latent $h[b, t+1, c]$.

Three categories of negatives are natural in this setup.
\begin{itemize}
\item \textbf{Cross-temporal.} Pairs spanning different time positions:
  past against future $\big(h[b, t, c],\, h[b, t+1, c]\big)$, past
  against forecast $\big(h[b, t, c],\, f[b, t, c]\big)$, or forecast
  at $t$ against forecast at $t+1$
  $\big(f[b, t, c],\, f[b, t+1, c]\big)$, with or without a channel
  switch. The current loss does not include these terms; an older
  variant did.
\item \textbf{Cross-channel.} Pairs at the same time from a different
  channel of the same series, $h[b, t, c']$ for $c' \neq c$. There are
  $C - 1$ such candidates per anchor.
\item \textbf{Cross-batch.} Pairs across different series in the
  batch, $f[b', t, c]$ for $b' \neq b$, paired against the future
  latent at $(b, t+1, c)$. There are $B - 1$ such candidates per
  anchor.
\end{itemize}
...
candidates are computed by a $B \times B$ inner-product grid over the
whole batch with the diagonal masked out. Both are single matrix
multiplications that the GPU executes in parallel. In the loss we do
not keep the per-anchor sets separate: at every position $(t, c)$ we
aggregate the candidates of all anchors into two batch-pooled sums,
and the same denominator is used for every anchor at that position.
The cross-batch pool is therefore quadratic in $B$ and dominates the
negatives.
...
\[
  P_+(b, t, c) \;=\; \sigma\!\big(f[b, t, c],\, h[b, t+1, c]\big),
\]
and the two batch-pooled negative sums at position $(t, c)$ are
\begin{align*}
...
This list is not exhaustive. One could draw negatives from a queue of
past batches to expand the pool beyond the current batch, or pair
channels across different series. Different families of time series
may benefit from specific extra negatives tailored to their structure,
and we have not explored this further.
