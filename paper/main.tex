...
For each training sample we draw an order $p, q \in \{1, \ldots, P\}$
uniformly at random, sample the AR and MA coefficients, and shrink
them in $\ell_1$ until the polynomial roots lie strictly outside the
unit disk (a standard stationarity constraint). The resulting
coefficient vectors are zero-padded up to length $P$, so that every
sample has a fixed total of $2P$ ground-truth coefficients regardless
of the realised orders. We evaluate two configurations: $P = 2$ (which
we call \emph{2x2}, $4$ coefficients per sample) and $P = 4$
(\emph{2x4}, $8$ coefficients per sample). The 2x2 configuration is a
smaller comparison point; 2x4 is our production setup.

A series is generated by simulating $T = 4096$ steps from the sampled
ARMA process. We use the simplification $F = 1$ and $C = 1$ (each
sample is a single univariate series). The series is then patched into
$T_p = T / W = 128$ patches of width $W = 32$ following
...
% TODO: regenerate the equivalent 4-coefficient scatter for the 2x2
% configuration. Pending GPU re-run with the trained backbone +
% recovery head on a fresh 2x2 test set.
For the 2x2 setup the head reaches an overall recovery ratio of
$8.34$, with per-coefficient correlations comparable to those reported
for 2x4. The corresponding 4-coefficient scatter figure is omitted for
the moment and will be added when regenerated.
...
depth-vs-shallow finding, and use $L = 12$ in our production runs as
