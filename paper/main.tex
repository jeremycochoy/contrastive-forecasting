...
$L^2$-normalise every latent so that $\|h[t, c]\| = 1$.

The choice of $W$ is dataset-dependent. Values in $\{16, 32, 64, 128\}$
all appear in the recent foundation-model literature and all work as
inputs to our encoder. In the limit, $W = 1$ is admissible; the model
reduces to per-step encoding. The stride between consecutive patches
is the patch width by default, since this minimises compute and avoids
redundant encoding work. A smaller stride down to $1$ is also
admissible if the loss is adjusted to align each forecast with the
correct future patch.
...
patched-decoder line and gives a serviceable baseline.
...
For every batch element $b$, every position $t$, and every channel $c$,
the positive pair is the forecast at $(b, t, c)$ matched against the
future latent at $(b, t+1, c)$. The negatives we use in production fall
into two categories.
\begin{itemize}
\item \textbf{Cross-channel.} The latent of patch $t$ from a different
  channel of the same series, $h[b, t, c']$ for $c' \neq c$. This
  contributes $C - 1$ negatives per anchor and is computed by a single
  $C \times C$ inner-product grid with the diagonal masked out.
\item \textbf{Cross-batch.} The forecast at $(b', t, c)$ for
  $b' \neq b$, paired against the future latent at $(b, t+1, c)$. This
  contributes $B - 1$ negatives per anchor and is computed by a single
  $B \times B$ inner-product grid with the diagonal masked out, in one
  matrix multiplication that the GPU executes in parallel.
\end{itemize}
The cross-batch term is the bulk of the negative pool, contributing
$B(B-1)$ pairs at every $(t, c)$ position from a single matmul and
giving us a quadratic amount of negatives essentially for free. The
cross-channel term is quantitatively smaller but informative, since the
channels of one series share global statistics yet differ in their
fine-grained dynamics.

Writing $\tau$ for the temperature, the contribution of one anchor
$(b, t, c)$ to the loss is
\begin{equation}
  \mathcal{L}_{b, t, c}
  \;=\; -\log\!\frac{\exp\!\big(s_+ / \tau\big)}
  {\sum_{\text{neg}} \exp\!\big(s_\text{neg} / \tau\big)},
  \label{eq:loss}
\end{equation}
where $s_+$ is the positive similarity at $(b, t, c)$ and the sum in
the denominator runs over the cross-channel and cross-batch negatives
of that anchor. We average over all anchors and use $\tau = 0.07$.
...
variants of all three. The earlier version of the loss carried these
terms explicitly. We removed them after observing that on series whose
consecutive patches are close in latent space (ARMA among them), the
temporal negatives are not far enough from the positive to provide
useful signal, and the extra cost is not repaid by what the model
learns at each step.
