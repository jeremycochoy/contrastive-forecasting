...
capacity. Each layer is a standard self-attention block followed by a
feed-forward network, with residual connections and layer
normalisations.

Between the attention output and the feed-forward block we insert a
small depthwise causal convolution (kernel size $3$) on the residual
stream. The motivation comes from the analysis of induction
heads~\citep{olsson2022inductionheads}: a short-range convolutional
component lets the model develop induction-head-like behaviour with
shallower attention than would otherwise be needed, which in practice
helps the forecaster learn faster.
